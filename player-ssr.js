/* ============================================================
   player-ssr.js — TRIAL FORK of player.js, loaded ONLY by topic-01.html.
   ------------------------------------------------------------
   Same player, but the sentence cards are NOT built from JS. The page
   ships them as static, source-visible HTML (collapsed accordion), and
   this fork HYDRATES them: reveal toggles a CSS stage class (st-0..st-3)
   instead of rebuilding innerHTML. Diffs vs player.js are marked "SSR FORK".
   player.js itself is untouched; every other topic page still uses it.
   Revert: point topic-01.html back at player.js and delete this file.
   ------------------------------------------------------------
...
(function () {
  ...
  var PLAYER_HTML =
    ...
    '</div>' +
    // SSR FORK: #sentence-list is NOT injected here — the page ships it as static,
    // source-visible cards (outside #player-root, so mount() never wipes them).
  ...
  // SSR FORK: the cards are static in the page. Instead of rebuilding innerHTML (which
  // would destroy the source-visible text), reflect each card's reveal stage onto its
  // class (st-0..st-3); CSS in the page <head> shows/hides the matching reveal rows.
  ...
  function render() {
    sentences.forEach(function (s) { syncCard(s.num); });
  ...
  function flagSent(e, num) {
    e.stopPropagation(); e.preventDefault();
    var a = window.ThaiEarAuth;
    // SSR FORK: the static flag button always calls flagSent; if not signed in, route to
    // the feature sign-in page (the old code used a separate logged-out button for this).
    if (!a || !(a.getUser && a.getUser())) return flagSignIn(e);
  ...
  // SSR FORK: reflect flag state onto the existing static buttons (don't rebuild the list,
  // which would wipe the source-visible text). Toggles the .flagged class + title only.
  function syncFlags() {
    var a = window.ThaiEarAuth;
    var loggedIn = !!(a && a.getUser && a.getUser());
    sentences.forEach(function (s) {
      var btn = document.querySelector('#sc-' + s.num + ' .sent-flag-btn');
      if (!btn) return;
      var flagged = loggedIn && a.isFlagged && a.isFlagged(TOPIC_KEY, s.num);
      btn.classList.toggle('flagged', !!flagged);
      btn.setAttribute('title', flagged ? 'Flagged — click to remove'
        : (loggedIn ? 'Flag this sentence' : 'Sign in to flag sentences'));
    });
  }

  // Load the user's flags once, then sync flag state onto the static buttons; re-run on auth.
  function initFlags() {
    var a = window.ThaiEarAuth;
    if (a && a.isReady && a.getUser && a.getUser() && a.loadFlags) {
      a.loadFlags().then(syncFlags).catch(function () {});
    } else {
      syncFlags(); // logged out (or auth gone) → flags show but route to sign-in
    }
  }
