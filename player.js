...
(function () {
  ...
  function dynChainPlayable(t, idx, fg) {
    ...
    /* WITH A BUILD (the foreground, fg): the clips still have to be gettable. Offline that means
       ON THIS DEVICE — otherwise the build has nothing to fetch and the hop stalls. Online is
       already covered by dynStdUsable above, so this clause only ever speaks for the offline case
       and never has to trust navigator.onLine. */
    if (fg) {
      ...
      if (t.dynKey && String(t.dynKey).indexOf('pl-') === 0) {
        try {
          var pm = JSON.parse(localStorage.getItem('thaiear_offline_pl') || '{}');
          if (pm[String(t.dynKey).slice(3)]) return true;
        } catch (_) {}
      }
    }
    return false;
