...
(function () {
  ...
  window.addEventListener('pageshow', function (e) {
    ...
    if (e && e.persisted) dynUnadoptQuiet();
  });
  ...
  function dynChainPlayable(t, idx) {
    ...
    /* ⚠ THIS IS THE LOCK-SCREEN PREDICATE — it is only consulted when !fg — so it must ask
       what can play WITHOUT BUILDING. "has a prefix" used to stand in for "has a placeholder, so
       it can always play"; offline that is false, because the combined track is network-only
       unless this device downloaded it. A downloaded topic does NOT rescue the backgrounded case:
       dynAdoptBuild refuses when hidden, so hopping onto it would produce the flash-and-revert
       this walk exists to avoid. Skipping is the honest outcome there.
       Online dynStdUsable() is true for every topic, so this is byte-identical to the old
       `return !!t.prefix` whenever there is a connection. */
    return dynStdUsable(t, currentMode);
  }
