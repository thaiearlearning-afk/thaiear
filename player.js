...
(function () {
  ...
  function makeNativeAudio() {
    ...
    var st = { src: '', preparedSrc: null, paused: true, currentTime: 0, duration: 0, seek: null };
    ...
    return {
      ...
      /* ⚠⚠ r219 — A SEEK ASKED FOR BEFORE THE NEW ITEM IS PREPARED IS HELD, NOT ISSUED.
         Until now it went straight to NA.seekTo, which moved the player still holding the
         OUTGOING media item; the prepare below then started the new one at 0 and the seek was
         simply gone. Deliberately NOT written into st.currentTime either — callers repair a lost
         seek by re-checking `currentTime < 0.5` after play() resolves, and writing the wanted
         position here is what made that check answer "we got there" over audio parked at 0:00. */
      set currentTime(v) {
        if (st.preparedSrc !== st.src) { st.seek = v; return; }
        st.currentTime = v; if (NA) NA.seekTo({ seconds: v });
      },
      get src() { return st.src; },
      /* ⚠⚠ r219 — AND A NEW src FORGETS THE OLD TRACK, BECAUSE THAT IS WHAT <audio>.load() DOES.
         This object used to carry the replaced track's currentTime AND duration into the next
         one, which is why the Android app alone could not resume a reconstruct (owner,
         2026-09-15: the iPhone PWA and desktop are a real element and were always right).
         `duration` was the sharper half of the two: dynAutoRebuild bounds its resume by it, so a
         stale — shorter — figure put the anchor "past the end" and the resume was abandoned
         before it was attempted. Raising the repeat count lengthens the session and pushes every
         sentence later, so the commonest settings change was the one guaranteed to trip it. On
         the web that clause reads NaN and is covered by the `!isFinite` test. */
      set src(v) { if (v !== st.src) { st.src = v; st.preparedSrc = null; st.currentTime = 0; st.duration = 0; st.seek = null; } },
      set preload(v) {},
      load: function () {},
      play: function () {
        ...
        return NA.prepare(prep)
          .then(function () {
            st.preparedSrc = st.src; st.paused = false;
            var s = st.seek; st.seek = null;
            if (!(s > 0)) return NA.play();
            /* The position asked for while this item was still loading. Media3 takes a seek on a
               buffering item, and PlaybackService's CMD_SEEK skips its clamp while getDuration()
               is still TIME_UNSET — so playback starts where the listener actually was. Play
               either way: a refused seek is a resume that lands at 0:00, not a silent player. */
            st.currentTime = s;
            return NA.seekTo({ seconds: s }).then(function () { return NA.play(); },
                                                  function () { return NA.play(); });
          })
          .catch(function (e) {
  ...
  function syncToPlayingTrack() {
    ...
    NA.addListener('time', function () {
      ...
      if (DYN) {
        ...
        /* ⚠⚠ `cfg.dynKey` ALONE IS NOT THIS PAGE'S KEY, AND ON A TOPIC IT IS NULL (r221).
           np.key is stamped from the CHAIN ENTRY's dynKey, which resolveDynChain() synthesises as
           the bare page id ('topic-07') for a derived chain. A PLAYLIST page sets cfg.dynKey and
           the two agree; a TOPIC or GRAMMAR page sets none — verified across the live pages — so
           `np.key !== null` made a page FOREIGN TO ITSELF, every time, and the else-branch below
           (commented "own unit playing") was unreachable on a topic. Same class as r214's
           namespace bug: a page-scoped question answered with the chain's key. */
        var ownKey = cfg.dynKey || (dynChain && dynChain[dynHomeIdx] ? dynChain[dynHomeIdx].dynKey : null);
        ...
        if (isForeign) {
          ...
          var dm = dynUnitNs(t) ? dynReadMeta(dynUnitNs(t), currentMode) : null;
          dynSession = dm ? { url: null, fileUri: null, blob: null, map: dm.map, key: dm.key, duration: dm.duration, display: true } : null;
          /* r221: DERIVED, not hard-coded. This branch is for a unit that is not ours, so it is
             false in every case the branch is meant for — but it was the SECOND half of the
             topic-highlights-nothing bug: reached wrongly (see ownKey above) it switched the
             highlight off while holding this page's OWN map. Asking the namespace makes the flag
             true to the map whatever route arrives here. Sound for topics too: no two units in
             topics.js share an `audio:` prefix, so namespace equality means the same unit. */
          dynSessionIsLocal = !!(dm && dynUnitNs(t) === DYN_KEY_NS);
          dynStdRemote = false;
  ...
  /* Web only: iOS hands a page back from the bfcache with its JS state intact, so an adopted
     neighbour would otherwise resume here — see dynUnadoptQuiet. NATIVE is excluded because there
     the engine really is still playing that unit and the adoption is still true. */
  window.addEventListener('pageshow', function (e) {
    if (NATIVE || !DYN) return;
    if (!(e && e.persisted)) return;
    /* ⚠ ONLY WHEN A NAVIGATION ACTUALLY HAPPENED. A FAILED one — the offline notice for an
       uncached page — must leave the adoption exactly as it was, or going back loses what was
       playing for a navigation that never occurred. See dynNavCompletedSinceAdopt. */
    if (dynNavCompletedSinceAdopt()) dynUnadoptQuiet();
    else dynLog('pageshow: keeping the adoption (no navigation completed)');
  });
  ...
  function cachePage() {
    /* r230: returns a promise that settles when the save and its stamp have (or failed), so a
       topic Update can wait for it before saying "downloaded". Never rejects. */
    if (!window.caches || !navigator.onLine) return Promise.resolve();
    try {
      return caches.open(DL_PAGE_CACHE).then(function (c) {
        c.add('/audio-versions.json').catch(function () {});
        if (PLMODE) { return c.add(location.href).catch(function () {}); }
        ...
        return fetch(key + '?te-save=' + Date.now(), { cache: 'no-store', credentials: 'same-origin' })
          .then(function (res) {
          ...
          .then(function (got) {
            ...
            return dynQzSigLoad().then(function (map) {
              ...
              return c.put(key, new Response(got.t, { headers: { 'Content-Type': got.ct } })).then(function () {
                ...
                /* ⛔⛔ r228 — THE PAGE STAMP IS WRITTEN HERE AND NOWHERE ELSE on the dyn path: the
                   hash of the bytes just saved, which equals the published pageVer by the check
                   above. dlFreshStamp holds the rendered-document comparison for the rest of this
                   page's life, because the DOM on screen may still be the pre-update page. */
                try {
                  var mf = getManifest(), en = mf[PREFIX];
                  if (en) { en.ver = h; setManifest(mf); dlFreshStamp = true; }
                } catch (_) {}
  ...
  /* dlLeaveOk: the learner has already said "Leave anyway" in OUR dialog, so the native one must
     not ask a second time on the unload that follows. */
  var dlLeaveOk = false;
  window.addEventListener('beforeunload', function (e) { if (downloadingNow && !dlLeaveOk) { e.preventDefault(); e.returnValue = ''; } });
  window.addEventListener('pageshow', function (e) { if (e && e.persisted) dlLeaveOk = false; });   // bfcache restore: re-arm
  /* ── r226: THE BACK-GESTURE GUARD ON A TOPIC PAGE ────────────────────────────────────────────
     ⭐ Map of every leave guard on the site: LEAVE_GUARDS.md (project root).
     Owner, 2026-09-24: "when doing a backswipe motion on android app mid topic download, i see a
     grey unstyled box about confirm navigation - i thought we had intercepting styled messages".
     We did — for a LINK TAP (installLeaveGuard, below) and, on My Playlists, for the swipe too
     (pl-list.js r93). The topic page never got the swipe half, so a back gesture fell straight
     through to `beforeunload`, whose dialog is the browser's own and cannot be styled. Nothing to
     do with connection speed: it happened on every back swipe during every topic download.
     Same mechanism as pl-list.js: while a download runs, one same-URL SENTINEL entry sits on top
     of the page's own. A back gesture pops it — same-document, so no unload and no native
     dialog — we put it back and ask in the site's own modal. "Leave anyway" goes back past both.
     ⚠ THREE THINGS THIS PAGE HAS THAT THE PLAYLIST PAGE DOES NOT:
     · quiz.js pushes its own { te_quiz } entry here. We act only on a pop that lands BELOW our
       sentinel (state not ours) with the quiz closed, and only ever pop an entry that is ours —
       a download finishing while the quiz sits on top leaves the sentinel ORPHANED rather than
       popping the quiz's entry; the next pop onto it just carries on backwards (dlGuardStale).
     · The learner scrolls while a topic downloads, and scrollRestoration 'auto' would put them
       back where they were when the download STARTED on every pop of the sentinel (theirs or
       ours). So the scroll is tracked while armed and re-applied after each pop — the scroll
       event lands a frame after popstate, so dlGuardY is still the learner's value there.
     · beforeunload must stand down once they have chosen to leave (dlLeaveOk). */
  var dlGuardArmed = false, dlGuardPushed = false, dlGuardSkip = false, dlGuardStale = false, dlGuardY = 0;
  function dlGuardOurs() { try { return !!(history.state && history.state.teDlGuard); } catch (_) { return false; } }
  function dlGuardPush() { try { history.pushState({ teDlGuard: 1 }, '', location.href); } catch (_) {} }
  function dlGuardQuizOpen() { return document.documentElement.classList.contains('tq-mode'); }
  function setDownloadingNow(b) {
    downloadingNow = b;
    if (b && !dlGuardArmed) {
      dlGuardArmed = true;
      dlGuardY = window.pageYOffset || 0;
      /* ⚠ Not while the quiz is open: a sentinel above ITS entry would be popped by the swipe
         that closes the quiz, and we could not tell that apart from leaving. beforeunload stays
         the backstop for that one rare case. */
      if (dlGuardOurs()) { dlGuardPushed = true; dlGuardStale = false; }   // reuse an orphan on top
      else if (!dlGuardQuizOpen()) { dlGuardPush(); dlGuardPushed = true; dlGuardStale = false; }
    } else if (!b && dlGuardArmed) {
      dlGuardArmed = false;
      if (dlGuardPushed) {
        dlGuardPushed = false;
        if (dlGuardOurs()) { dlGuardSkip = true; try { history.back(); } catch (_) { dlGuardSkip = false; } }
        else dlGuardStale = true;                                     // the quiz is on top: leave it be
      }
    }
  }
  window.addEventListener('scroll', function () { if (dlGuardArmed || dlGuardSkip) dlGuardY = window.pageYOffset || 0; }, { passive: true });
  window.addEventListener('popstate', function () {
    var keepY = function () { var y = dlGuardY; try { window.scrollTo(0, y); requestAnimationFrame(function () { window.scrollTo(0, y); }); } catch (_) {} };
    if (dlGuardSkip) { dlGuardSkip = false; keepY(); return; }
    if (dlGuardOurs() || dlGuardQuizOpen()) return;       // the quiz's own entry was popped: not ours
    if (dlGuardStale) {                                   // an orphaned sentinel: keep going back
      dlGuardStale = false;
      try { history.back(); } catch (_) {}
      return;
    }
    if (!dlGuardArmed || !dlGuardPushed) return;
    dlGuardPush();                                        // cancel the gesture: back onto the sentinel
    keepY();
    dynConfirm('Download in progress',
      'If you leave now, this download won’t finish. You can start it again any time.',
      'Leave anyway', 'Keep downloading', function () {
        dlLeaveOk = true;
        if (dlGuardArmed && dlGuardOurs()) { dlGuardArmed = dlGuardPushed = false; try { history.go(-2); } catch (_) {} }
        else { try { history.back(); } catch (_) {} }     // finished while the dialog was open
      });
  });
  ...
  function dynCheckPageUpdate() {
    ...
    if (!dlFreshStamp && e.ver && e.ver !== contentHash()) { updPage = true; paintUpdatePrompt(); return; }
  ...
  function dynPlSaveQuiz() {
    ...
    /* ⛔⛔ r228 (sw v693) — TWO FAULTS IN THIS WRITER, BOTH FIXED HERE (DOWNLOAD_UPDATE_PATH.md fault 10).
       1. It read the stamps as `sig[u]` from dynQzSigLoad(), which since r217 returns the WHOLE
          published manifest ({units, sig, ver}), so every lookup missed and the record got
          qz: {} — which both the playlist page and the list read as STALE. Owner, 2026-09-24:
          "I see dotted update circle, hit update, then when I go to playlist menu, still says
          dotted circle". No number of Updates could clear it.
       2. It fetched the side-cars THROUGH sw.js (the fault 9 shape: a 2 s fallback can answer
          from thaiear-audio-dl itself) and stamped them whether or not the saved file was the
          published one.
       ✅ ThaiEarDL.saveQuizSidecars() is the ONE implementation both playlist writers use: it
       fetches past the worker, hashes each file against the published sig, writes only matches,
       and returns exactly the units it saved — the only ones stamped. */
    var DL = window.ThaiEarDL;
    if (!DL || !DL.saveQuizSidecars) return;
    return dynQzSigLoad().then(function (map) {
      var sig = map && map.sig;
      if (!sig) return;                       // nothing published: leave the record without a stamp
      return DL.saveQuizSidecars(AUDIO_DL_CACHE, units, sig).then(function (saved) {
        try {
        ...
        /* v694: repaint from the record just written, so a bar that had to fall back to
           "update" before the stamp landed corrects itself without a reload. */
        try { if (!downloadingNow) renderOfflineBar(); } catch (_) {}
      });
    }).catch(function () {});
  }
  /* r215 — record the published quiz stamp for THIS unit against its prefix. Safe on the
     ...
     download current. */
  /* ⭐ r225 — set the moment finalize stamps a PUBLISHED baseline, and never cleared: it lives
     only in memory, so the next navigation starts false by construction. It suppresses exactly
     one thing — the rendered-document comparison below — because right after an update the
     bytes in the cache are current while THE DOM ON SCREEN IS STILL THE OLD PAGE, and comparing
     against it would re-raise the prompt we just satisfied. The PUBLISHED comparison still runs,
     so a genuinely stale unit is still caught; only the weaker local proxy is held. */
  var dlFreshStamp = false;
  function dynStampUnitQz() {
    ...
    return dynQzSigLoad().then(function (map) {
      ...
      try {
        ...
        e.qz = sig[unit];
        /* ⛔⛔ r225 — AND `ver`, FROM THE SAME FETCHED MAP, IN THE SAME WRITE. THE SIXTH
           DIVERGENCE WAS THIS LINE'S ABSENCE. Finalize also sets `e.ver = contentHash()`, which
           hashes THE DOCUMENT CURRENTLY RENDERED — and at the moment Update is pressed that is
           still the OLD page, because nothing has reloaded. So the baseline was stamped with the
           hash of what we were replacing, while dynCheckPageUpdate() compares it against
           `pver[unit]`, the PUBLISHED hash. The two can never agree, so the prompt returned on
           every visit and NO number of re-downloads could clear it — exactly what the owner saw
           on topic-13b, while `qz` (fetched, current) cleared normally on the same units.
           ✅ Writer and reader now read the same field of the same fetched map. `contentHash()`
           stays as the synchronous provisional value for the gap before this resolves, and as
           the fallback when the map cannot be fetched at all.
           ⚠ This is a WRITER fix, so it reaches the page and the card together by construction —
           both read these same manifest fields. That is why it does not violate §0. */
        /* ⛔⛔ r228 (sw v693) — `ver` IS NO LONGER WRITTEN HERE. r225 wrote it from the published
           map, which fixed the epoch split but still stamped a page as current whether or not its
           bytes had been saved: the owner's phone showed Getting to know you "quiet" with its
           saved copy OLD, because the save had been refused (bytes != published) while this line
           recorded success anyway. STAMPS ARE NOT BYTES. `ver` is now written by cachePage(),
           and only after the bytes it saved hash to the published pageVer, so a failed or refused
           save leaves the card, the page and the offline copy all saying "update" — which is true
           — and pressing Update again retries. r225's property holds in a stricter form: the
           stamp equals the published hash whenever it is written. */
        setManifest(m);
      } catch (_) {}
    }).catch(function () {});
  }
  ...
  function dynDownloadHere() {
    ...
    if (!navigator.onLine) { setDownloadingNow(false); offBarLock = false; latMark('dl:SKIP', 'navigator.onLine false'); offlineBarFlash('offline'); return; }   // don't grind through retries
    var by = dynDlGroups(), prefixes = Object.keys(by), total = 0, done = 0;
    prefixes.forEach(function (k) { total += by[k].files.length; });
    if (!total) { setDownloadingNow(false); offBarLock = false; latMark('dl:SKIP', 'nothing to fetch'); renderOfflineBar(); return; }
    ...
    chain.then(function () { return loadAudioVers(); }).then(function (avMap) {
      ...
      var qzStampP = null;   // r230: the quiz stamp write, awaited before "downloaded" (strict mode: must be declared)
      prefixes.forEach(function (pfx) {
        var existed = !!m[pfx];
        ...
        if (!PLMODE && ref === 'topic') qzStampP = dynStampUnitQz();   // r230: awaited before "downloaded"
      ...
      if (PLMODE) {   // so the playlists page's own Clear knows which prefixes to release
        try {
          ...
          var qzSave = dynPlSaveQuiz();
        } catch (_) {}
      }
      /* ⛔ v694 (r229) — A PLAYLIST UPDATE IS NOT DONE UNTIL ITS QUIZ IS SAVED AND STAMPED. The
         record above is rewritten WITHOUT `qz`, and this used to announce "downloaded" at once
         while dynPlSaveQuiz() was still fetching: any repaint in that gap read the missing stamp
         as stale and flipped back to "update", and leaving the page before the save finished
         meant the stamp never landed. Owner, 2026-09-24, a premium playlist on a slow link:
         "id hit page update, then it would revert back immediately … tried a few times until it
         took". So hold the downloading state until the save settles, capped so a hung fetch can
         never strand the bar; whatever lands after the cap still stamps and repaints itself. */
      function finishDl(pageSaved) {
        stampVerified();
        if (!pageSaved) cachePage();
        setDownloadingNow(false);
        offBarLock = false;
      ...
      if (PLMODE && typeof qzSave !== 'undefined' && qzSave && qzSave.then) {
        return Promise.race([qzSave.catch(function () {}),
                             new Promise(function (r) { setTimeout(r, 45000); })]).then(function () { finishDl(false); });
      }
      /* ⛔ r230 (sw v696) — THE TOPIC TWIN OF v694. A topic Update said "downloaded" as soon as the
         audio landed, while the verified page save (which alone writes `ver`) and the quiz stamp
         ran in the background. Until they finished, the published check saw the old `ver` and
         flipped the bar back to "update"; leaving in that gap left the offline copy old.
         Owner, 2026-09-24: a Buddhism unit took two presses without leaving the page, and he
         asked the right question — "does a topic or playlist now say downloaded only after all
         files are present and stamp is made? otherwise user could leave early". Now: wait for
         both, capped at 45 s so a hung fetch cannot strand the bar. */
      if (!PLMODE && ref === 'topic') {
        var pageSaveP = cachePage();
        return Promise.race([Promise.all([pageSaveP, qzStampP]).catch(function () {}),
                             new Promise(function (r) { setTimeout(r, 45000); })]).then(function () { finishDl(true); });
      }
      finishDl(false);
    }).catch(function (err) {
      setDownloadingNow(false);
  ...
  var STYLES = `
    ...
    /* --font-ui (Inter) carries no Thai glyphs, so a Thai NAMED PAIR inside a gloss
       -- e.g. "owing to (เนื่องจาก…จึง)" on the grammar units -- would otherwise resolve in
       whatever Thai face the OS supplies, beside the Sarabun of the .g-thai chip next
       to it. Listing --font-thai second is per-character fallback: Latin stays Inter. */
    .g-eng { font-family: var(--font-ui), var(--font-thai); color: var(--text-secondary); }
  ...
  /* sp = THAI PLAYBACK SPEED. Per unit, like every other setting here — the sync button is what
     propagates a setup, and speed is no different (owner, 2026-09-16).
  ...
  /* ⛔ SP_STEPS IS THE ONE SOURCE FOR THE OFFERED SPEEDS — the UI slider, the settings restore
     and the sanitiser all read it. It was three separate literals until 2026-09-19, which is
     exactly how a stop can exist in one place and not another.
     ⚠ 0.84 IS THE FLOOR AND IT WAS RAISED FROM 0.80 ON PURPOSE (owner, 2026-09-19): at 0.80 a
     long sentence dragged badly, and "I'd rather sentences can't get as slow as people might
     like than have them sound poor quality" — the Thai is a paid studio recording and stretch
     artefacts are what a listener hears, not the speed they asked for. Do not lower it back. */
  var SP_STEPS = [1, 0.92, 0.84];     // index 0 = Normal, 2 = slowest offered
  /* Snap any stored value onto the offered stops. ⚠ LOAD-BEARING WHEN THE STOPS CHANGE: a
     preference saved under the old set (0.9 / 0.8) is still a valid-looking number, so without
     this the slider falls back to index 0 and reads "Normal" while the audio keeps building at
     the old rate. Nearest-stop preserves the intent — whoever picked the slowest keeps getting
     the slowest. */
  function dynSnapSp(v) {
    if (!isFinite(v) || v <= 0) return 1;
    var best = SP_STEPS[0];
    for (var i = 1; i < SP_STEPS.length; i++) {
      if (Math.abs(SP_STEPS[i] - v) < Math.abs(best - v)) best = SP_STEPS[i];
    }
    return best;
  }
  ...
  function dynNormSet(o) {
    ...
    /* The floor was set by ear on real clips (owner, 2026-09-16, listening to the stretch at
       0.7/0.8/0.9: "70 doesn't sound good, and neither will 75"), then raised to 0.84 on
       2026-09-19 for the same reason one step further in. ⚠ The ACCEPTED range stays 0.8–1
       while the OFFERED set is SP_STEPS: a record written by an older build (0.9 / 0.8) must
       still be recognised as a real choice rather than reset to normal, and dynSnapSp() then
       moves it onto the nearest current stop. Widening acceptance is safe in this direction
       only — same reasoning as the r147 pause-floor note above. */
    var sp = parseFloat(o.sp); if (isFinite(sp) && sp >= 0.8 && sp <= 1) d.sp = dynSnapSp(sp);
    return d;
  }
  ...
  /* ⭐ WHOSE SETTINGS ARE THESE CONTROLS SHOWING? (owner, 2026-08-27.)
     *"the dyn player settings for a given topic page should OWN that even if navigating from
     ...
     dyn player settings that are visible should show the configuration of topic B even though on
     topic A's page, then if hit 'Return' you go back to the dynamic mp3 for topic A AND dyn
     player is showing topic A's settings. this way no reconstruction is needed and conflict is
     avoided and so is user confusion."*
     Exactly right, and it is the same root as the rebuild: settings are ALREADY stored per unit
     (dynSetKey(ns, mode)) — nothing was making the ACTIVE namespace follow the adopted unit. So
     the player built B with A's numbers, wrote a session under a key B's own page would never
     compute, and showed A's controls the whole time.
     The settings panel controls the PLAYER, and the player is playing the adopted unit; the
     sentence cards below it are still this page's content, which is unchanged and correct.
     ⚠ EXCLUSIONS DO NOT TRAVEL. They are a property of the card list on screen, which is always
     this page's own — so they stay on DYN_KEY_NS. Only the four playback settings move. */
  function dynActiveNs() {
    return dynAdopted ? (dynUnitNs(dynAdopted) || DYN_KEY_NS) : DYN_KEY_NS;
  }
  ...
  function dynSaveSettings() {
    var o = dynCurrentSet();
    o.ts = Date.now();
    /* ⚠ WRITES FOLLOW WHAT IS DISPLAYED. Showing B's settings while an edit silently changed A's
       would be a worse confusion than the one this fixes — the controls and the store have to
       name the same unit. */
    dynWriteJson(dynSetKey(dynActiveNs(), currentMode), o);
    dynPrefsQueue('set');
  }
  ...
  function dynEnsureMainSrc(lenient) {
    ...
    /* ⚠⚠ THESE TWO GUARDS ARE ABOUT **THIS PAGE'S OWN** PLAYLIST, SO THEY MUST STAND DOWN WHILE
       A NEIGHBOUR IS ADOPTED (2026-08-27). They ask "is there anything in the playlist this page
       represents", which is exactly the right question when the page is playing itself and
       entirely the wrong one when the player is on another unit. Offline on a playlist page whose
       own clips are not downloaded, they fired BEFORE the adopted branch below could run — so
       hopping to a neighbouring playlist and changing a setting reported "Nothing in this playlist
       is downloaded yet" about a playlist nobody was listening to, and the r217 revert never got
       the chance. That is why the fix verified on favourites and grammar (topic units, no PLMODE)
       and not on the playlists loop. */
    if (!dynAdopted && PLMODE && sentences.length && !dynIncluded().length && sentNoDlCount()) {
      dynStatus('Nothing in this playlist is downloaded yet', false);
      return Promise.reject({ code: 'nothing-playable' });
    }
    if (!dynAdopted && PLMODE && sentences.length && !dynIncluded().length) {
    ...
    if (dynAdopted) {
      ...
      return dynResolveAdopt(adoptedT).catch(function (e) {
        /* ⚠⚠ THE ADOPTED BRANCH HAD NO FALLBACK AT ALL, and r215 routed a settings change through
           it (2026-08-27). Owner's lead, and it is the right one: *"this mechanism broke because we
           now call auto reconstruction of dyn mp3s when settings toggle — before you would hit
           play, and you'd get the message 'can't reconstruct'"*. r18e's revert lives in the LOCAL
           session path's catch; the adopted path never had one, so a rebuild that cannot run
           offline simply rejected and left the visitor with nothing — no revert, no message worth
           reading, and the session that WOULD play still sitting in the cache.
           Same shape as r18e, same 1200 ms, and deliberately only when there is something to fall
           back to: a stored session for the unit that is actually playing. */
        if (isGateCode(e && e.code) || !dynReadMeta(dynUnitNs(adoptedT), currentMode)) return Promise.reject(e);
        dynStatus('Couldn’t rebuild — putting your settings back…', false);
        return new Promise(function (res) { setTimeout(res, 1200); }).then(function () {
          if (dynAdopted !== adoptedT) return Promise.reject(e);
          if (!dynRevertToStored()) return Promise.reject(e);
          dynAdoptStale = false;               // the reverted settings ARE the stored session's
          dynStatus('No connection — your settings have been put back to the version saved on this device.', false);
          dynRevertMsgUntil = Date.now() + 6000;
          var rseq = dynStatusSeq;
          setTimeout(function () { if (rseq === dynStatusSeq) { dynRevertMsgUntil = 0; dynStatus(null); } }, 6000);
          return dynResolveAdopt(adoptedT);
        });
      }).then(function (r) {
        if (dynAdopted !== adoptedT || !r) return;
        dynSession = r.sess;
        dynStdRemote = r.std;
        ...
        dynSyncSentBtns();
        /* ⚠ THE LOCAL PATH CLEARS THIS AND THIS ONE NEVER DID. dynAutoRebuild() puts
           "Re-constructing dynamic mp3" up before calling in here; when the resolve was satisfied
           from a cache it returned without a build, so nothing took the message down and it sat
           there for ever. Clearing on success covers every route into this branch.
           ⚠ …EXCEPT WHEN THE MESSAGE IS THE REVERT'S OWN EXPLANATION. The revert resolves THROUGH
           this branch, so an unconditional clear here erased the one thing the visitor needed to
           read. */
        if (Date.now() >= dynRevertMsgUntil) dynStatus(null);
      });
    }
    ...
    return dynEnsureSession(function (done, total) {
      ...
      /* ⚠⚠ THE REVERT IS TRIED BEFORE ANY MESSAGE IS CHOSEN, AND 'nodl' USED TO JUMP THE QUEUE
         (2026-08-27, owner: *"when im playing a cached topic offline … if i alter the settings of
         the dyn player e.g. change repeats … all it does is break"*).
         That is the commonest offline shape there is, because a BUILT SESSION and DOWNLOADED CLIPS
         are different things: the session blob lives at dynCachePath(), the clips in the
         thaiear-audio-dl cache, and a topic played online then revisited offline has the first and
         not the second. Change a setting there and the new key needs a rebuild, the rebuild needs
         clips, and every one of them is missing → 'nodl' → the early return below fired, which
         both skipped the revert entirely and, on a TOPIC page, said "Nothing in this playlist is
         downloaded yet". Meanwhile a perfectly playable session for the previous settings was
         sitting on the device.
         So: a session already on the device is a better answer than any message. Only when there
         is nothing to fall back on does the failure get reported. */
      var stored = dynReadMeta(DYN_KEY_NS, currentMode);
      if (!stored) {
        /* Nothing to fall back on. NOW the code decides the wording — and 'nodl' means every clip
           was unavailable with no entitlement problem, i.e. the not-downloaded case reached via
           layer 2 because navigator.onLine claimed to be online. "Check your connection" would be
           the wrong advice there: what the visitor needs is the download. */
        if (code === 'nodl') {
          dynStatus(PLMODE ? 'Nothing in this playlist is downloaded yet'
                           : 'This topic isn’t downloaded — reconnect to build it', false);
        } else {
          dynStatus('Couldn’t load the audio — check your connection', false);
        }
        return Promise.reject(e);
      }
      // r18e: the build failed, but there IS a playable session on the device, built with
      // DIFFERENT settings. Say so briefly, then put the settings back to that session and play it.
      dynStatus('Couldn’t rebuild — putting your settings back…', false);
      return new Promise(function (res) { setTimeout(res, 1200); }).then(function () {
        if (!dynRevertToStored()) {
          // The stored key could not be parsed back onto the controls — report the real failure
          // rather than leaving a "putting your settings back" that never happened.
          dynStatus('Couldn’t load the audio — check your connection', false);
          return Promise.reject(e);
        }
        dynStatus('No connection — your settings have been put back to the version saved on this device.', false);
  ...
  /* ⚠ dynActiveNs(), NOT DYN_KEY_NS. Since r214 the settings panel and dynSaveSettings() follow
     the ADOPTED unit, so the revert has to read the same namespace or it would put back a
     neighbour's settings from this page's stored session — or, more usually, find nothing at all
     and report failure while a perfectly good session sat on the device under the other key. */
  function dynRevertToStored() {
    var meta = dynReadMeta(dynActiveNs(), currentMode);
    ...
    dynSpeed = (isFinite(k.sp) && k.sp >= 0.8 && k.sp <= 1) ? dynSnapSp(k.sp) : 1;
    ...
    /* ⚠⚠ EXCLUSIONS ARE THIS PAGE'S, AND AN ADOPTED KEY'S NUMS ARE NOT. The key's num list is
       the included set — true, and useful when the session being reverted to is OUR OWN. When the
       player is on an adopted neighbour those numbers belong to a different unit entirely, and
       writing them here would silently exclude every sentence on the page the visitor is looking
       at. Only restore exclusions when the active unit IS this page's. */
    if (!PLMODE && k.nums && dynActiveNs() === DYN_KEY_NS) {
      var inc = {};
    ...
    dynSaveSettings();
    dynPrefsRepaintControls();
    /* ⚠ dynKey() DESCRIBES THIS PAGE'S UNIT — its sentence numbers, not the adopted one's — so
       comparing it to an adopted session's key is always false and would report a successful
       revert as a failure. (It did: the settings came back but the caller reported failure and
       left "Couldn't rebuild…" on screen.) For our own unit the full key is the honest check,
       because the nums are ours; for an adopted one, the settings ARE the whole of what we can put
       back, and applying them is the whole of what was asked. */
    if (dynActiveNs() !== DYN_KEY_NS) return true;
  ...
  /* Set when a settings change has invalidated the ADOPTED unit's built session, so the next
     resolve must BUILD rather than hand back either cache. Cleared the moment a build lands. */
  var dynAdoptStale = false;
  /* While this is in the future the status line is explaining a REVERT and must not be cleared.
     ⚠ r213 added an unconditional dynStatus(null) to the adopted branch, so that a stale
     "Re-constructing dynamic mp3" could not outlive a resolve that never built anything. Correct
     — and it also wiped the revert's own explanation, because the revert RESOLVES through that
     same branch. Two fixes, one status line: the same collision offBarHoldUntil settles for the
     offline bar, settled the same way. */
  var dynRevertMsgUntil = 0;
  /* ⭐⭐ A SETTINGS CHANGE WHILE ADOPTED BELONGS TO THE ADOPTED UNIT (2026-08-27, owner:
     *"if i changed the settings whilst its playing, it shows 'constructing dynamic mp3...'
     forever, then when i go onto that topic, it reconstructs as per the OLD not changed
     settings"*).
     dynInvalidate() asked all of its questions about THIS PAGE'S unit — dynKey(),
     dynReadMeta(DYN_KEY_NS) — while the thing actually playing was a neighbour. Two failures fell
     out of that:
       · THE HANG. The rebuild reached dynEnsureMainSrc's adopted branch, dynResolveAdopt handed
         back the session sitting in dynAdoptCache — built moments earlier under the OLD settings
         — and nothing on that path clears the status line, so "Re-constructing dynamic mp3" sat
         there for ever over audio that had never been rebuilt.
       · THE STALE BUILD. Because no new session was ever built or persisted, the unit's own page
         later restored the OLD one (its restore is deliberately lenient — round-10 item 3), so
         the change the visitor had just made appeared to be forgotten.
     The owner's read was right: this cannot be owned by the page, because the page is not what
     is playing. Route it to the unit that IS. */
  function dynInvalidateAdopted(fromControl) {
    var t = dynAdopted;
    dynLog('invalidate (adopted ' + (t && (t.prefix || t.dynKey)) + ')');
    dynAdoptStale = true;                     // force a real build past both caches
    var entry = dynAdoptCache[t.page];
    if (entry) {
      if (entry.sess && entry.sess.url && entry.sess !== dynSession) { try { URL.revokeObjectURL(entry.sess.url); } catch (_) {} }
      entry.sess = null;
    }
    var wasPlaying = (!mainAudio.paused && !mainOnSilence()) || dynAutoResume;
    if (!mainAudio.paused) { mainAudio.pause(); setMainIcon(false); }
    dynLastPos = 0; dynPosStale = true; dynAttached = false; mainSrcReady = false;
  ...
  function dynInvalidate(fromControl) {
    if (dynAdopted) { dynInvalidateAdopted(fromControl); return; }
    var key = dynKey();
  ...
  var dynPrefsTimer = null, dynPushGdef = false, dynPushUnit = false, dynPushNs = null;
  ...
  function dynPrefsQueue(which) {
    ...
    /* ⚠ CAPTURE THE UNIT NOW, NOT WHEN THE TIMER FIRES. The push is debounced 1000 ms and a
       chain hop can land inside that window, which would file this edit against whichever unit
       happened to be adopted a second later. */
    if (which !== 'gdef') dynPushNs = dynActiveNs();
    var a = window.ThaiEarAuth;
    ...
    dynPrefsTimer = setTimeout(function () {
      ...
      if (dynPushUnit) { dynPushUnit = false; api.set(dynPushNs || DYN_KEY_NS, dynUnitPayload()); }
    }, 1000);
  }
  /* r223 — the slider track is a gradient we paint (see the CSS note), so the fill point has to
     follow the value. ⚠ CALL IT AFTER EVERY WRITE TO .value, not just on user input: the account
     sync, a TE/ET switch and an adopted unit all repaint these controls, and a fill left at the
     previous value is the same class of confusion r214 was about. Reading min/max off the element
     keeps it honest if a range is ever re-scaled. */
  function dynFillSlider(el) {
    if (!el) return;
    ...
    try { el.style.setProperty('--te-fill', Math.max(0, Math.min(100, pct)) + '%'); } catch (_) {}
  }
  function dynPrefsRepaintControls() {
    ...
    var spEl = $('dyn-sp');
    if (spEl) { var i = SP_STEPS.indexOf(dynSnapSp(dynSpeed)); spEl.value = String(i < 0 ? 0 : i); dynFillSlider(spEl); }
    var reps = $('dyn-reps');
  ...
  var plysDwell = null;   // { num, ms, last, need, ths, rp, run, banked, heard, sent, lastT }
  /* How far the playhead must jump BACKWARDS inside one block before it is read as a replay
     rather than as playback. timeupdate fires about every 250 ms and the playhead only ever
     advances while audio runs, so anything beyond a tick is a seek; 350 ms leaves room for the
     tick cadence without letting a real rewind slip through. The controls that produce one are
     nowhere near the threshold — dynSentSkip(-1) restarts the block only when it is already
     1.5 s in, and the ⟳ repeat loop jumps back the whole block. */
  var PLYS_REWIND_S = 0.35;
  ...
  function plysDwellTick(num, b, t) {
    ...
    /* ⚠⚠ A REPLAY OF THE SAME BLOCK IS MORE LISTENING, NOT THE SAME LISTENING (owner,
       2026-08-27): *"three thai repeats, i have listened to two to completion, then i hit back to
       start of the block and listen to the sentence 3 more times — the sentence count only
       increments by 3"*. Five repetitions were heard and three were credited.
       Cause: `heard` was a running max over the whole VISIT to the block, capped at `rp`. Seeking
       back to the start does not change `num`, so the same dwell record survives, byPos climbs
       from zero again — and the cap made `rp` the ceiling for the visit rather than for the pass
       through it. ⟳ REPEAT was in the same shape: loop a sentence ten times at rp=1 and it
       credited one.
       So a backwards jump BANKS what this run earned and starts a fresh run. The cap stays where
       it belongs — on the run, which is what `rp` actually describes.
       ⚠ THE WALL CLOCK IS RESET WITH IT, AND THAT IS THE ANTI-INFLATION HALF. Each new run has
       to bank its own `need` per repetition out of real timeupdate ticks, so seeking back and
       forth without playing credits nothing, exactly as a scrub still credits nothing. It also
       discards whatever fraction of the next repetition had accrued — deliberate: under-crediting
       a partial repetition is the safe direction, and this counter's whole design prefers it.
       ⚠ STILL ONE PASS. `plysDwellFlush()` emits once, when the block is left, carrying however
       many repetitions were heard — the server contract is unchanged (/api/plays reads reps[k]
       only where deltas[k] > 0), passes stay ≤ reps, and the topic roll-up's "complete listens"
       does not move because someone replayed a sentence. */
    if (t < plysDwell.lastT - PLYS_REWIND_S) {
      plysDwell.banked += plysDwell.run;
      plysDwell.run = 0;
      plysDwell.ms = 0;
    }
    plysDwell.lastT = t;
    ...
    var run = Math.max(0, Math.min(plysDwell.rp, Math.min(byPos, byTime)));
    if (run > plysDwell.run) plysDwell.run = run;
    var heard = plysDwell.banked + plysDwell.run;
  ...
  /* ── DID A NAVIGATION ACTUALLY COMPLETE? ─────────────────────────────────────────
     r214 un-adopts on a bfcache restore, because iOS hands a page back with dynAdopted intact and
     it would otherwise resume a neighbour the visitor had walked away from. Right for the case it
     was written for — and WRONG for one it cannot see (owner, 2026-08-27): offline, he tapped the
     Now-playing strip to open the adopted topic's own page, the page was not cached, he got the
     offline notice, and going back dropped the adoption. He never left: the navigation FAILED.
     Nothing about a failed forward navigation says "I am done listening to this".
     The two cases are distinguishable, and cheaply. `te_mount` records WHEN a player page last
     mounted in this tab. A real navigation mounts a page; the SW's offline fallback is a
     self-contained HTML string with no player.js (sw.js offlinePage()), so it mounts nothing and
     leaves the marker where it was.
     ⚠ The question is "did ANY page mount after we adopted", not "did the TARGET page mount" —
     A→B→C then back to A is still a real navigation, and asking about B specifically would keep
     a stale adoption there.
     ⚠ sessionStorage, not localStorage: this is a fact about THIS TAB's history, and it must not
     leak between tabs or outlive the session. */
  var dynAdoptedAt = 0;
  function dynMarkMounted() {
    try { sessionStorage.setItem('te_mount', String(Date.now())); } catch (_) {}
  }
  function dynNavCompletedSinceAdopt() {
    if (!dynAdoptedAt) return true;          // never adopted here — nothing to protect
    try {
      var t = parseInt(sessionStorage.getItem('te_mount') || '0', 10);
      return !!t && t > dynAdoptedAt;
    } catch (_) { return true; }             // cannot tell → keep r214's behaviour
  }
  /* ⚠⚠ WHERE A CHAIN UNIT'S PER-UNIT STORAGE ACTUALLY LIVES — AND IT IS NOT ITS `dynKey`
     (2026-08-27). A page namespaces its exclusions, settings and persisted sessions on
     `DYN_KEY_NS = cfg.dynKey || PREFIX`. A PLAYLIST page sets `cfg.dynKey` ('pl-<id>'), so the
     chain entry's dynKey and the page's namespace agree. **A TOPIC PAGE SETS NO dynKey AT ALL** —
     verified across the live topic-*.html — so its namespace is its PREFIX, while the chain
     entry's dynKey is the bare page id that resolveDynChain() synthesises ('topic-07').
     Every adopt-path lookup used the chain's dynKey, so for a topic it read and wrote a namespace
     that unit's own page never looks at. Consequences, all of which were live:
       · a persisted session was never found — dynReadMeta() always missed, which is why a topic
         adopt always fell through to the prefab and nobody noticed while the prefab worked;
       · r213's build then PERSISTED under that same wrong namespace, so opening the unit's own
         page rebuilt from scratch a session already on the device (owner, on the "Now playing"
         link: *"the dyn player starts reconstructing the dyn mp3 rather than just seamlessly
         continuing play"*);
       · dynSettingsFor() read the wrong namespace too, so the build ignored the listener's own
         settings for that unit — defeating the point of building rather than using the prefab.
     One helper, used by every per-unit read and write in this path. */
  function dynUnitNs(t) {
    if (!t) return null;
    if (t.dynKey && String(t.dynKey).indexOf('pl-') === 0) return t.dynKey;   // playlists: page sets cfg.dynKey
    return t.prefix || t.dynKey || null;                                      // topics: DYN_KEY_NS === PREFIX
  }
  function dynPrefetchNeighbours() {
    ...
    [dynChainStep(-1), dynChainStep(1)].forEach(function (stp) {
      ...
      var ns = dynUnitNs(t);
      var meta = ns ? dynReadMeta(ns, mode) : null;
      if (meta) dynRestoreSession(ns, mode, meta).then(function (sess) { if (sess) entry.sess = sess; }).catch(function () {});
    });
  }
  ...
  function dynChainPlayable(t, idx, fg) {
    if (idx === dynHomeIdx && dynSession && dynSessionIsLocal) return true;
    var pns = dynUnitNs(t);
    if (pns) {
      var c = dynAdoptCache[t.page];
      if (c && c.sess) return true;
      if (dynReadMeta(pns, currentMode)) return true;   // best sync guess (restore may still miss)
    }
    /* CAN THIS UNIT PRODUCE AUDIO AT ALL? Two answers, because the two callers ask different
       questions — `fg` is what separates them.
       WITHOUT BUILDING (the lock screen, !fg): a persisted session, or a usable placeholder.
       "has a prefix" used to stand in for the latter; offline that is false, because the combined
       track is network-only unless this device downloaded it. A downloaded topic does NOT rescue
       the backgrounded case — dynAdoptBuild refuses when hidden — so skipping is honest there.
       ⚠ Online dynStdUsable() is true for every topic, so with a connection this is byte-identical
       to the old `return !!t.prefix` in BOTH modes. The change is offline-only, by construction. */
    if (dynStdUsable(t, currentMode)) return true;
    /* WITH A BUILD (the foreground, fg): the clips still have to be gettable. Offline that means
       ON THIS DEVICE — otherwise the build has nothing to fetch and the hop stalls.
       ⚠ THE SENTENCE THAT USED TO END THIS NOTE — "online is already covered by dynStdUsable
       above, so this clause never has to trust navigator.onLine" — WAS TRUE ONLY FOR TOPICS, and
       believing it of playlists is r222's bug. See the pl- branch. */
    if (fg) {
      if (t.prefix && isDownloaded(t.prefix)) return true;
      // A playlist spans prefixes and has no `prefix` of its own; its own download record is the
      // equivalent claim (the same map pl-list.js and dynDlWasDownloaded read).
      if (t.dynKey && String(t.dynKey).indexOf('pl-') === 0) {
        if (dynPlDownloaded(t)) return true;
        /* ⚠⚠ AND ONLINE A PLAYLIST IS BUILDABLE EXACTLY AS A TOPIC IS (r222). The ONLY clause in
           this function that means "online, so it can be built from nothing" is dynStdUsable(),
           and that opens `if (!t.prefix) return false` — a playlist has no prefix, so it never
           reaches the navigator.onLine line that makes every topic playable. The foreground walk
           therefore skipped every never-constructed playlist and hopped to the next CONSTRUCTED
           one, while the identical walk over topics always builds (owner, 2026-09-19).
           ⚠ NOBODY CHOSE THAT. The pl- test above was written in r216 for the OFFLINE case, where
           "downloaded" really is the equivalent of a topic's clips being on disk, and it reads as
           if it were the whole answer. It was only ever half of one.
           The preconditions are what a build actually needs, not a proxy for them: the items on
           this device (the same local cache dynChainSentences reads) AND at least one of them
           entitled — so r216's rule still holds, and the walk never hops onto a unit that can
           only fail. navigator.onLine LIES in the WebView; that is survivable here for the same
           reason it is for topics, which have relied on it since r212 — dynAdvance's catch retries
           the build locally once, and that retry is extended to playlists in this same build. */
        if (fg && navigator.onLine && dynPlBuildable(t)) return true;
      }
    }
    return false;
  }
  /* Does a FOREIGN playlist unit hold a download? The page-scoped dynDlWasDownloaded() answers
     this for cfg.dynKey only, which is the wrong unit on every chain path — hence the separate
     reader rather than a call into it. Extracted from dynChainPlayable in r222 because the retry
     in dynAdvance's catch needs the same answer, and a second inline copy of a localStorage shape
     is how the tier-rule divergences started. */
  function dynPlDownloaded(t) {
    var k = String((t && t.dynKey) || '');
    if (k.indexOf('pl-') !== 0) return false;
    try { return !!JSON.parse(localStorage.getItem('thaiear_offline_pl') || '{}')[k.slice(3)]; }
    catch (_) { return false; }
  }
  /* Can a foreign playlist be BUILT right now from what this device holds? Its items must be in
     the local playlist cache, and at least one of them must be playable by this visitor.
     ⚠ Entitlement goes through dynUnitLocked() — the ONE shared predicate (ThaiEarAuth.lockedFor).
     Do NOT reimplement the tier rules here; a divergent copy is what produced bug #7 and §B4. */
  function dynPlBuildable(t) {
    var items = dynChainSentences(t);
    if (!items || !items.length) return false;
    for (var i = 0; i < items.length; i++) {
      if (!dynUnitLocked({ tier: items[i].tier || 'free', prefix: items[i].prefix })) return true;
    }
    return false;
  }
  ...
  function dynTopicSentences(t) {
    ...
    return T.loadSentenceNums().then(function (map) {
      ...
      /* ⚠ THE TARGET UNIT'S OWN EXCLUSIONS, not this page's. dynExcluded is module state for the
         page we are standing on; the unit being adopted keeps its own list under its own key, and
         its page will apply that list when it computes its session key. Ignore them here and the
         key we persist can never match the one that page looks for. */
      var excl = {};
      try {
        var raw = JSON.parse(localStorage.getItem('te_dyn_excl_' + dynUnitNs(t)) || '[]');
        if (Array.isArray(raw)) raw.forEach(function (n) { excl[n] = true; });
      } catch (_) {}
      var keep = nums.filter(function (n) { return !excl[n]; });
      if (!keep.length) return null;
      return keep.map(function (n) {
  ...
  /* ⭐ BUILD THE UNIT, AND ONLY FALL BACK TO THE PREFAB WHEN A BUILD IS IMPOSSIBLE
     (owner, 2026-08-27). The combined `_TE`/`_ET` track is a pre-dyn artefact: fixed order, both
     languages, and NONE of the listener's dyn settings — no repeats, no pause length, no
     exclusions. Hopping onto it meant prev/next quietly played something different from what the
     same topic plays when you open its page.
     ⚠ A HYBRID WAS CONSIDERED AND REJECTED BY THE OWNER: start the prefab instantly, swap to
     the built session when it is ready. *"the prefab would start, then once the dyn mp3 is
     generated the first sentences would start playing again."* Exactly right — the swap restarts
     the audio mid-listen, which is worse than a few seconds of "Constructing…". So the build is
     unconditional in the foreground and the wait is shown honestly.
     ⚠ THE PREFAB IS NOT DEAD, and must not be deleted: a LOCK-SCREEN hop cannot build (WebKit
     suspends media loading for a backgrounded page, and dynAdoptBuild refuses when hidden), so it
     is still the only source there. It is also the fallback when a build fails online — audio the
     listener did not ask for beats silence — but NOT when offline, where a remote URL cannot
     resolve and reporting the failure honestly is the only truthful option.
     ⚠ A build PERSISTS its session (dynPersistSessionFor), so the second hop onto the same unit
     resolves instantly from disk. The wait is paid once per unit per settings change. */
  function dynAdoptBuildElsePrefab(t, mode) {
    return dynAdoptBuild(t).catch(function (e) {
      if (isGateCode(e && e.code)) return Promise.reject(e);   // a paywall is not a load failure
      if (!dynStdUsable(t, mode)) return Promise.reject(e);    // no prefab available — say so
      dynLog('adopt: prefab (build unavailable)');
  ...
  function dynResolveAdopt(t) {
    ...
    var mode = currentMode;
    var c = dynAdoptCache[t.page];
    /* ⚠ …UNLESS A SETTINGS CHANGE JUST INVALIDATED IT. Both caches hold audio built under the
       OLD settings, and handing either back is what made the change look forgotten. */
    if (dynAdoptStale) {
      dynLog('adopt: build (settings changed)');
    ...
    var ns = dynUnitNs(t);
    var meta = ns ? dynReadMeta(ns, mode) : null;   // synchronous pre-check (stale-lenient by design)
    ...
    if (!meta) {
      dynLog('adopt: build (no persisted session)');
      return dynAdoptBuildElsePrefab(t, mode);
    }
    return dynRestoreSession(ns, mode, meta).then(function (sess) {
      if (sess) { dynLog('adopt: restored persisted'); return { src: (NATIVE && sess.fileUri) ? sess.fileUri : sess.url, std: false, sess: sess }; }
      dynLog('adopt: build (restore miss)');
      return dynAdoptBuildElsePrefab(t, mode);
    });
  }
  ...
  /* ⚠⚠ COMING BACK TO THIS PAGE MUST NOT RESUME THE TOPIC WE LEFT IT FOR (2026-08-27, owner:
     *"when i nav back to the topic page i was on before (back swipe, iOS) it starts playing from
     the OTHER topic"*).
     Adoption is a claim that another unit's audio is playing THROUGH this page's transport. In the
     native app that is literally true — the engine keeps running across a navigation, which is why
     syncToPlayingTrack() exists and why the state is worth restoring. **On the web it is not.** A
     navigation tears the page down and the audio with it; iOS then hands the page BACK from the
     bfcache with every variable exactly as it was, `dynAdopted` included. So the page woke up still
     believing it was playing the neighbour, still sourced to that neighbour's blob, and resumed it
     — on the very page the visitor had just navigated *away* from it to.
     Un-adopt quietly on a persisted restore: come home, but do NOT play. The visitor arrived here
     by going back, not by pressing play, and dynReturnLocal() would start audio they did not ask
     for. Everything below is dynReturnLocal's state reset, shared rather than copied. */
  function dynUnadoptQuiet() {
    if (!dynAdopted) return;
    dynLog('unadopt (restored from bfcache)');
    try { if (!mainAudio.paused) mainAudio.pause(); } catch (_) {}
    setMainIcon(false);
    dynResetToLocal();
  }
  function dynResetToLocal() {
    dynAdopted = null;
    ...
    dynSession = null;     // the neighbour's session is not ours to keep playing
    ...
    // ↩ Return brings THIS page's own settings back with it (dynAdopted is null by now).
    dynLoadSettings(); dynPrefsRepaintControls();
  }
  ...
  function dynApplyAdoptState(t) {
    ...
    /* ⭐ THE CONTROLS FOLLOW THE UNIT (owner, 2026-08-27). dynAdopted is set just above, so
       dynLoadSettings() now resolves the TARGET's namespace — the panel shows B's repeats, pause
       factor and English toggle while B is what is playing, and dynAdoptBuild() builds with those
       same numbers. Before this the panel showed A's while B played, and the build used A's, which
       is why the session it stored could never match the one B's own page looks for. */
    dynLoadSettings(); dynPrefsRepaintControls();
  ...
  function dynAdvance(t, revertIdx) {
    ...
    dynAdoptedAt = Date.now();      // see dynNavCompletedSinceAdopt
    ...
    dynResolveAdopt(t).then(function (r) {
      ...
      /* ⚠ r222 — AND A PLAYLIST NEEDS THIS RETRY FOR THE SAME REASON A TOPIC DOES. The clause
         read `t.prefix && isDownloaded(t.prefix)`, and a playlist has no prefix, so the retry was
         unreachable for one. That was survivable only while the walk refused to hop onto an
         unbuilt playlist at all; now that it does (dynChainPlayable's pl- branch), the case this
         retry exists for — navigator.onLine lying in the WebView — reaches playlists too, and
         without it a downloaded playlist would revert instead of rebuilding from disk. */
      if (!dynAdoptRetried && dynAdopted === t && document.visibilityState === 'visible' &&
          !isGateCode(e && e.code) && !(e && (e.code === 'licence' || e.code === 'nosess')) &&
          ((t.prefix && isDownloaded(t.prefix)) || dynPlDownloaded(t))) {
  ...
  installLeaveGuard(function () { return downloadingNow; }, function (proceed) {
    dynConfirm('Download in progress',
      'If you leave now, this download won’t finish. You can start it again any time.',
      'Leave anyway', 'Keep downloading', function () { dlLeaveOk = true; proceed(); });
  });
  ...
  var DYN_STYLES =
    ...
    /* ⚠⚠ THE TRACK IS PAINTED BY US, NOT DERIVED FROM accent-color (r223). With `accent-color`
       alone, BLINK derives the unfilled track from the accent — and for the premium GOLD it
       derives NEAR-BLACK, so the bar right of the thumb was black in the Android app while the
       iPhone (WebKit ignores the accent for the track) stayed neutral grey. Measured in Chrome
       151, not reasoned: a darker gold (#B29234) is ALSO black, and `color-scheme: only light`
       does not change it — so neither of the two obvious one-line fixes works. One manual track
       serves both tiers; the non-premium purple renders identically to the UA default it
       replaces, which is why this is not tier-keyed. --te-fill is kept in step by
       dynFillSlider(); its 50% fallback only ever shows if that never runs. */
    '.dyn-slider input[type=range]{width:105px;accent-color:var(--accent);height:3px;padding:9px 0;box-sizing:content-box;touch-action:none;-webkit-appearance:none;appearance:none;background:transparent}' +
    '.dyn-slider input[type=range]::-webkit-slider-runnable-track{height:3px;border-radius:2px;background:linear-gradient(to right,var(--accent) 0 var(--te-fill,50%),var(--border-strong,rgba(0,0,0,.18)) var(--te-fill,50%) 100%)}' +
    '.dyn-slider input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:15px;height:15px;border-radius:50%;background:var(--accent);margin-top:-6px}' +
    '.dyn-slider input[type=range]::-moz-range-track{height:3px;border-radius:2px;background:var(--border-strong,rgba(0,0,0,.18))}' +
    '.dyn-slider input[type=range]::-moz-range-progress{height:3px;border-radius:2px;background:var(--accent)}' +
    '.dyn-slider input[type=range]::-moz-range-thumb{width:15px;height:15px;border:0;border-radius:50%;background:var(--accent)}' +
    ...
    if (row) {
      ...
      sl.innerHTML = '<span class="dyn-ctl-group">' + dynInfoLabel('Pauses', 'pauses') +
        ...
        /* THAI SPEED. Named for what it affects, because it affects only that (owner,
           2026-09-16). The slider carries no number: the underlying values are SP_STEPS, but a
           percentage invites picking a figure rather than listening, so the ends are labelled
           ...
           ⚠ Because no number is shown, changing SP_STEPS needs no copy change anywhere. */
      ...
      dynFillSlider(pf);
      pf.addEventListener('input', function () { pv.textContent = (parseFloat(pf.value) || 1) + '×'; dynFillSlider(pf); });
      ...
      // ⛔ Reads the module-scope SP_STEPS — do NOT reintroduce a local copy here.
      function spIdx(v) { var i = SP_STEPS.indexOf(dynSnapSp(v)); return i < 0 ? 0 : i; }
      spEl.value = String(spIdx(dynSpeed));
      dynFillSlider(spEl);
      /* ⚠ `input` ONLY REPAINTS — it must never write or invalidate. The `change` handler below is
         deliberately the only writer (dragging across stops would otherwise queue a rebuild per
         stop), and the fill still has to track the thumb while the finger is down. */
      spEl.addEventListener('input', function () { dynFillSlider(spEl); });
  ...
  function advanceTopic(dir) {
    ...
    if (DYN) {
      ...
      while (guardN-- > 0) {
        ...
        /* ⚠⚠ AND "CAN IT BE BUILT" IS NOT UNCONDITIONALLY TRUE IN THE FOREGROUND EITHER
           (2026-08-27, owner: *"if i favourite a topic that is not downloaded and never a dyn
           session constructed (no cache) it tries to play it — it should SKIP if no playable
           audio, not choke."*). This line used to read `fg || dynChainPlayable(…)`, on the premise
           stated just above that anything can be built in the foreground. That is true ONLINE and
           false OFFLINE: with no clips on the device and no persisted session there is nothing to
           build FROM, so the walk hopped onto a unit that could only fail — and it fails as a
           STALL, not an error, which is why it reads as a choke.
           Exactly the same hoist as the entitlement check above, for the same class of reason, and
           it lands on all four loops (topics, playlists, grammar, favourites).
           ⚠ NOT gated on navigator.onLine, which this file says LIES in the WebView in three
           places. dynChainPlayable() already asks the honest question; it simply was not being
           asked. */
        if (dynChainPlayable(stp.t, stp.idx, fg)) { hop = stp; break; }
        dynLog('chain skip ' + (stp.t.dynKey || stp.t.prefix) + ' (nothing playable)');
      }
  ...
  function mount() {
    ...
    dynMarkMounted();   // "a player page loaded in this tab" — see dynNavCompletedSinceAdopt
