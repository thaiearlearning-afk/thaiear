/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v658';   // v658: SCROLL POSITION SURVIVES A BACK NAVIGATION on every topic listing. Owner, 2026-09-23: "on android app (not iphone pwa - this is fine). when i go into a topic, then back swipe gesture, i am always deposited at the top of that given menu ... annoying for users trying to explore and browse topics" -- and the same leaving through the Quiz yourself strip. Covers the five band pages, /topics, the grammar hub and Favourites, because they all run topics-page.js. ⚠⚠ DELIBERATELY NOT DIAGNOSED AS A WEBVIEW QUIRK: iOS restores and Android does not, which is a bfcache difference -- WebKit restores the whole document while the Android WebView reloads and leans on best-effort scroll restoration once fonts and late layout have moved things. Establishing which would cost a day and the answer would still be "do it ourselves", so this stops depending on the browser rather than trying to make the browser behave. ✅ ADDITIVE AND IDEMPOTENT -- it never touches history.scrollRestoration, because quiz.js's own notes record how easily that setting gets stuck and a stuck 'manual' would break scrolling everywhere. If the browser already restored correctly the scrollTo is a no-op; if it landed at 0 this corrects it. ⛔ ONLY ON A BACK/FORWARD navigation -- a fresh visit or a reload must start at the top, or a shared link drops a stranger into the middle of the grid. Saved on pagehide AND visibilitychange (iOS skips pagehide) AND on the click that leaves through a card or the quiz strip, which is the only moment guaranteed to happen on a fast WebView navigation. Restored three times -- immediately, across two rAFs and after load -- because the document grows after parse as the Thai face loads and decorate() adds the tick, pill and caption. sessionStorage, capped at 12 paths. ⭐ 5 new assertions, 2 red on the pre-fix file; the harness had to be given a navigation type and a scrollTo stub first, because jsdom reports neither and every one of them would otherwise have passed VACUOUSLY -- the third time that trap has bitten this week. topics-page.js is precached. v657 is spent.
