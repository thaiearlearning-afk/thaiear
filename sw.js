/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v590';   // v590: the quiz picker showed — instead of the learner's percentages on a device that had never played. quiz.js is precached and cache-first, so this bump delivers it; v589 is spent. ⚠⚠ THE TELL WAS THAT MOBILE WAS FINE AND DESKTOP WAS NOT, and it is the whole diagnosis: bestScore() reads the LOCAL mirror, so the phone (which had played) never needed the network while the desktop depended entirely on pull() landing. A bug that is per-DEVICE rather than per-account is a local-cache bug every time. ⛔ pull() returns false IMMEDIATELY when currentUser is null — it does not wait and does not reject — so the single attempt fired at open() could resolve false a few ms into the page load, the repaint guard would skip, and nothing ever corrected the dashes. syncScores() now retries on thaiear:auth, which auth.js fires when the session resolves. ✅ LATCHED ON SUCCESS ONLY: thaiear:auth fires once per RECORDED PLAY, so an unlatched retry would be a network round trip per sentence heard (the progress.html trap in CLAUDE.md). A failed pull costs nothing — it returns before touching the network — and the first success stops it for good; measured in a browser at 2 pull calls across 6 auth events. ⚠ Guarded on the quiz having been opened at all, so a learner who never touches the quizzes never pays for a pull. ⚠ PREDATES the v589 page migration — the logic was byte-identical before it — but ?quiz= urls made it far easier to hit by opening the quiz AT PAGE LOAD, the worst possible moment for an unresolved session.
