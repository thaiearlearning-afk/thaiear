/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v657';   // v657: THE PAGE READS THE PUBLISHED HASH TOO -- v656 gave that comparison to the CARD ALONE and the fifth divergence followed within the hour. Owner: time & numbers showing 'update available' on the card and 'downloaded' inside the topic. ⚠⚠ THE CARD WAS RIGHT. contentHash() measures THE DOCUMENT CURRENTLY RENDERED, so when the stale cached copy is what is being served its hash equals the stale `ver` and the page concludes it is current -- blind in precisely the case that matters. The published hash does not depend on which copy we happen to be looking at, so the page now checks it too; contentHash() is KEPT as the earlier-warning superset, firing the moment a fresh page is served without waiting for the map. ⛔ I BROKE §0 OF DOWNLOAD_UPDATE_PATH.md TO CAUSE THIS -- 'a new staleness signal goes into EVERY surface in the same change, or into none'. Adding it to one surface is the fault, and it does not stop being the fault when the addition is itself a fix for the previous divergence. ⭐ THE OWNER PANEL PROBE WAS A THIRD OPINION and is fixed in the same change: it compared only the quiz sig, so it printed 'quiet' for a unit the card was correctly flagging. It now shows sig AND pageVer and applies the same predicate. ⭐⭐ THE TEST THAT MATTERS: every previous stale_matrix row was satisfiable by contentHash() alone, so the page's new comparison ran VACUOUSLY. The added row makes the local document AGREE with the recorded baseline while the PUBLISHED hash disagrees -- unsatisfiable without consulting the published value, and exactly the owner's case. It fails against the pre-fix player.js and passes after. player.js is precached; ownersim.js is not. v656 is spent.
