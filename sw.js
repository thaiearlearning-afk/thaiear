/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v596';   // v596: 'My results' gains an AVG column before BEST — lifetime correctness. quiz.js and quiz.css are precached and cache-first, so this bump delivers them; v595 is spent. Owner, 2026-09-21: "if i have 23 right and 6 wrong, it shows my overall percentage of correctness of all time." ✅ NO NEW DATA AND NO SCHEMA CHANGE: `answered` and `correct` in resultsAll() are already the per-row sums of quiz_item_stats.seen/.correct, so AVG is exactly right/(right+wrong) — the two columns already on screen either side of it — derived at render and never stored, so it cannot drift from them. ⚠⚠ AVG AND BEST ANSWER DIFFERENT QUESTIONS AND WILL OFTEN DISAGREE, which is the point of showing both: BEST is the high-water mark of ONE run (quiz_scores is a MAX, §8.2) and never falls; AVG is every question ever answered and does. ⛔ 0/0 renders an em dash, not NaN% — reachable on a unit with a stored best but no item stats. ⚠ 'Listening comprehension' -> 'Listening' in THIS TABLE ONLY, via a new `shortName` field: the name cell is white-space:nowrap so the longest name sets the width and squeezes six numeric columns. ⛔ NOT a rename — the picker rows, entry tiles, quiz menu and results header all still say 'Listening comprehension', and an assertion pins shortName to its single use. Verified in a browser at 389px: table 341px in a 373px sheet, no overflow, no sideways page scroll, with 3-digit counts and a 100% best.
