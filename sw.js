/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v589';   // v589: THE QUIZ IS A VIEW ON THE TOPIC PAGE, NOT AN OVERLAY (owner, 2026-09-21). quiz.js and quiz.css are precached and cache-first, so this bump is what delivers them; v588 is spent. The quiz was `position: fixed; inset: 0` with a scrim and its own scroll container, sized against the viewport. Every desktop tweak that made it read as a page was a disguise fighting that shape. Now: the topic content is hidden by RULE (`body.tq-mode > *:not(nav.site-nav):not(#tq-page):not(.te-consent)`), the quiz renders into the page, the site nav stays visible and the document scrolls. The url carries `?quiz=<key>` via pushState and reopens that quiz's MENU on reload — never a RUN, which has nothing stored to restore from (§8.2). ⚠⚠ A RULE, NOT AN ELEMENT LIST, and that is load-bearing: topic pages have no <main>, and player.js appends TEN body-level elements at runtime. The floating mini transport is the one that would have bitten — fixed at top:54px, it latches once the topic audio plays and shows whenever #player-root is out of view, and display:none reads as out-of-view, so it would have slid in OVER the quiz. The old z-index:900 overlay covered it; nothing covers it now. ⛔ The false modal is gone with the overlay: role=dialog/aria-modal claimed a focus trap that never existed (zero occurrences of `focus` in 142KB of quiz.js), so a keyboard user could tab out into the frozen page. A page needs no dialog semantics. ⚠ A link click mid-run now warns — capture-phase on document, because nav.js REBUILDS the nav on every auth change and per-link handlers would be thrown away. ⚠ overscroll-behavior-y: contain kills pull-to-refresh for the duration, the one accidental reload a click handler cannot see. ⚠ The phone `min-height` SURVIVES on purpose — the flexible gaps need slack to grow into, and without it the picker collapses to its 6px minimums and undoes the 2026-09-20 spacing pass.
