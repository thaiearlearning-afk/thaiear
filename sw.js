/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v643';   // v643: TWO THINGS. (1) ⭐ v641's PAGE-UPDATE PROMPT REACHED NOBODY, and the owner found it on the Android app the same day: "i went in to feelings and emotions 1 on v641 and its showing me the old site page and 'downloaded' - im not seeing the page update available". dynCheckPageUpdate() read `if (!e.ver) return` -- "no baseline, no nag", copied from the AUDIO rule where it is right. It is exactly backwards for CONTENT: `ver` was never written by the dyn download path until v641, so ver:'' is the state of EVERY download that existed when the feature shipped -- i.e. the entire population it was built for. The two baselines are not symmetrical and that is the lesson: an absent AUDIO stamp means "we have never watched this" and the bytes are fine; an absent CONTENT stamp means the download predates the stamp, which is precisely when the page is most likely to be missing something. An absent baseline now reads as STALE, which is also the one-time flip the owner asked for ("can we just do a manual push this time so all downloads automatically flip - and from now on its automatic?"): self-clearing, because the update records the hash and every v641+ download records one at finalize. ⛔ Guarded so it cannot fire where there is nothing to deliver -- a page with no quiz block stays silent. ⚠⚠ MY OWN HARNESS HAD PINNED THE BUG AS CORRECT ("no ver baseline: no page-update nag"); a test written from the same wrong premise as the code cannot catch the code, and it only turned red once a real device disagreed with it. (2) THE TOPIC-CARD QUIZ STRIP IS 25% TALLER, READS "Quiz yourself", AND IS A REAL LINK to <unit>?quiz=menu (owner picked mock variant D — ⛔ "but no arrow", so no chevron goes back in). ⭐ IT WAS NOT ACTUALLY CLICKABLE WHERE IT MATTERED: the click handler lived inside topics-page.js's `if (!strip)` branch, which runs only for strips IT builds -- so on the five band pages and the grammar hub, which ship their own from the generator, tapping the strip did nothing. The same branch added `has-quiz`, so --strip-h stayed 0 and the favourites heart and download tick sat ~12px low, ON the strip -- the fault v608 fixed for the runtime path and never reached the static one. Both are now properties of the MARKUP (an <a>, and has-quiz emitted by cardHtml), so they cannot be missed on a surface again. ⚠ The strip is a SIBLING of the stretched card link, never nested -- which is what makes an anchor legal here. ⚠⚠ BOTH --strip-h-on REGIMES ARE MEASURED NET CARD GROWTH, NOT ARITHMETIC: 35.16px desktop and 30.95px phone. Adding the strip's own +6.99px to the old values gave 36 and 32 and left the heart 0.42px and 0.53px high -- an estimate wrong by less, not a fix. Measured in Chrome at 400px and 340px; drift is now 0.00 and -0.01px. Strip height 34.95px vs 27.95px = exactly +25.0%. ⛔ PLAYLISTS ARE UNTOUCHED and have no strip, by design (owner: "they dont even have a my quiz results band!! and shouldnt") -- test_pl_quiz pins it. player.js, topics.js, topics-page.js/css, the five band pages and grammar.html are all precached and cache-first. v641 is spent; v642 is a PEER'S, taken while origin, HEAD and disk all still read v641 - the constant on a shared tree answers a different question from the one you need.
