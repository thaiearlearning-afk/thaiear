/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v591';   // v591: QUIZ AUDIO NEVER USED THE DOWNLOADED COPY — quizClipUrl read a DIFFERENT offline key from the one the download wrote. player.js is precached and cache-first, so this bump delivers it; v590 is spent. Both keys are namespaced BY PREFIX — native offlineDir(pfx)+'/'+file, web webCacheKey(pfx,file) — and quizClipUrl hand-built 'offline/'+ref.file and '/__offline-audio/'+ref.file, dropping the prefix directory on BOTH paths. fromDisk() could therefore never match a downloaded clip and EVERY quiz clip fell through to the network. ⚠⚠ ONLINE THAT IS INVISIBLE, which is why it survived: it just fetches, with a small delay. Offline it is silence and a stuck play button. Owner, 2026-09-21, on the Android app with topic-22a downloaded: "on the vocab builder quiz, after i submit the result, i hit the player and the play button is just stuck, no audio. when online it fetches the audio but i think from online because slight delay." THE DELAY WAS THE DIAGNOSIS — a downloaded clip does not go to the network. ✅ Now uses the same helpers every other reader uses. ⚠ ref.prefix is PER SENTENCE (s.prefix || PREFIX): a playlist mixes topics, so the page's own PREFIX is the wrong namespace there. ⛔ PRE-EXISTING, NOT from the v589 page migration — that commit touched no audio code at all (verified by diff). It survived because offline had never been run end to end (§11A item 7), which is exactly what the owner was running when he hit it.
