/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v650';   // v650: TWO SPACING/SIZING FIXES THE QUIZ LINK EXPOSED, plus one word on the guide. (1) THE topic-meta LINE WAS 1px OFF THE PLAYER. Owner: "its basically touching the top of the dyn player". Measured at a true 390px viewport before touching anything: the line now WRAPS TO TWO because v644 added "Take the quiz" to it, and its bottom margin was 0.08rem -- 1.28px -- so the link's underline landed on the player card's top edge. Margin goes 0.1rem -> 0.5rem desktop and 0.08rem -> 0.45rem mobile, measured back as a 1px -> 7px gap, on all 113 unit pages. Deliberately small: these margins were CUT on purpose in an earlier pass to claw back ~50px above the fold, and this must not undo that. Vertical, so rem is allowed -- the px-only rule in TEXT_SCALING.md is for HORIZONTAL gutters. (2) FAVOURITES NOW EQUALISES ACROSS SECTIONS. Owner: the pills "are same as each other but DIFFERENT between sections". That page renders ONE .topic-grid PER SECTION -- Grammar by Ear, then one per band -- and grid-auto-rows:1fr equalises rows WITHIN a grid and never across two. Measured: short-named units 73px, long-named 91px, on one screen. MERGING INTO ONE GRID IS NOT AVAILABLE, and that was measured rather than assumed: the band headings are interleaved, and a full-width <h2> inside grid-auto-rows:1fr is STRETCHED to a whole card row (59px against 59px cards in a test grid), so one grid would leave a card-sized hole under every heading. So the grids stay separate and share a MEASURED height published as --tp-fav-card-h on #tp-fav-root. Verified 73/91 -> 92/92, stable across four consecutive runs, and the property is CLEARED when the list is empty. NOT the hand-written min-height topics-page.css forbids: that one encodes a line count and is wrong at every width but the tested one; this is the real rendered maximum, re-measured on resize, on visibilitychange and on thaiear:auth -- the last because decorate() adds the tick, the entitlement pill and the listening caption AFTER the first measurement, and each changes card height. It releases the property to 0 before measuring, or every pass would read back the height it imposed and the cards could only grow. (3) guide.html: the quizzes are "another way" to find out whether it has stuck, not "where" you find out -- they ADD to the active recall that English-first already asks for rather than superseding it (owner). topics-page.css, topics-fav.js and guide.html are precached and served cache-first. The 113 unit pages are not precached, but they are network-first and the CSS that lays them out is. v649 is spent.
