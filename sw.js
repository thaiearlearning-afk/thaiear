/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v647';   // v647: THE HOME SPLASH STOPS GUESSING AT OS TEXT SCALING -- HOMEPAGE_REDESIGN_2026-08-21.md 12.4, the last thing on that page that guessed, open since 2026-08-23. The descriptions under Topics/Playlists/Read Thai were hidden by (max-width:700px) and (max-height:639px), and a media query reads the VIEWPORT while Android textZoom and iOS Dynamic Type inflate TEXT without changing it -- so the rule could not see the case it exists to prevent. MEASURED, NOT REASONED (_ui-harness.html, 360 wide, gap from .s1 .s-meta bottom to the mark top; negative = collision): at h=640 it is +31 at 1x but -1 at 1.4x, -8 at 1.75x and -26 at 2x; at h=760, +42 / +11 / +4 / -14. So 639 was right ONLY at default text: at 640 -- the lowest height it called safe -- the descriptions already touch the mark at 1.4x, which is Android Large, and by 2x they collide at every phone height up to ~800. THE FIX IS A PRE-PAINT PROBE, because var() is not legal in a media query so this cannot be CSS at all, and nav.js publishes --te-ui-raw only AFTER first paint (using it would show the descriptions and then snatch them away). An inline script at the top of the stage measures a 100px/1 box with offsetHeight -- never getComputedStyle, which reports the SPECIFIED size -- and stamps html.te-nosub when innerHeight < 639 * raw. PROVED IN THREE PARTS: offsetHeight reads the inflation back exactly (1.15/1.40/1.75/2.00); the scaled threshold covers every height measured to collide; and at raw 1 the stamp fires on exactly the media query condition -- verified identical at 560/600/638 (hidden) and 660/640/700/760/844 (shown), with the 1x gaps unchanged at 31/45/42/63. THE MEDIA QUERY IS KEPT AND IS NOT REDUNDANT: it is the no-JS path and the fallback if the probe throws. STILL UNEXERCISED: a real device applying textZoom to the probe own element -- the harness inflates only elements that existed when __zoom ran, and the probe creates its own afterwards, so it legitimately measures 1 there. index.html is precached and cache-first. v646 is spent.
