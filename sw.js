/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v592';   // v592: THE REAL CAUSE of offline quiz silence — v591 fixed only half of it. player.js is precached and cache-first, so this bump delivers it; v591 is spent. quizClipUrl's fromDisk() REIMPLEMENTED localBlobUrl() and cachedBlobUrl() by hand and got BOTH copies wrong, so quiz audio never once used a downloaded clip on either platform since the quiz shipped. ⛔ FAULT 1, the one v591 MISSED and the one that mattered on Android: the native branch was gated on `window.ThaiEarDL`, which DOES NOT EXIST ON A TOPIC PAGE — dl-core.js is loaded by playlists.html, not by topic-NN.html (verified in a browser: undefined, and absent from the script list). So `cap` was always null, the native branch was UNREACHABLE, and the app fell through to Cache Storage where native downloads never go because those clips are on the FILESYSTEM. ⛔ FAULT 2, fixed in v591: the web key dropped the prefix directory, so the PWA branch could not match either. ⚠⚠ ONLINE BOTH ARE INVISIBLE — fromDisk() returns null and the clip is just fetched, with a delay nobody would question; that is why it survived. Owner, Android app, topic-22a downloaded: "the play button is just stuck, no audio. when online it fetches the audio but i think from online because slight delay" — THE DELAY WAS THE DIAGNOSIS. ✅ Now delegates to the two helpers the dyn player already uses, branching on player.js's own NATIVE flag, which has no page dependency. ⚠ ref.prefix is PER SENTENCE: a playlist mixes topics. ⛔ PRE-EXISTING, not from the v589 page migration — that commit touched no audio code.
