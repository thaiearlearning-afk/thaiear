/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v598';   // v598: the display name is OFFLINE-FIRST. auth.js and account.html are precached and cache-first, so this bump delivers them; v597 is spent. It was the ONE mutation in auth.js not following that file's own rule — it upserted to `profiles` and updated the local copies only AFTER the server answered. Owner on the Android app, 2026-09-21: offline it threw "TypeError: failed to fetch" and saved nothing; on a weak connection it "hung for ages and ages"; and the Save button stayed "a paler blue and not clickable" afterwards. ⛔ THE BUTTON FAULT NEEDED NO NETWORK AT ALL: the re-enable sat at the END OF THE PROMISE CHAIN, so a promise that never settles never ran it, and cancelEditName() only toggles display — so the SAME disabled button came back and nothing short of a reload cleared it. ✅ Now: apply locally and resolve in the same tick, queue the DESIRED FINAL VALUE uid-scoped, flush on the 'online' event AND on auth resolution (that event fires only on a TRANSITION, so an app queued-offline then reopened already-connected would never replay — the trap auth.js already documents for the playlist outbox). Last-write-wins, which is what the owner asked for and what progress/flags/dyn_prefs already do. ⛔ NOT modelled on the play counter, which queues DELTAS precisely because last-write-wins would lose plays. ⚠⚠ refreshProfile() now YIELDS to a pending rename: it runs on every auth resolution and applies the server's display_name, so without that guard a name typed offline would be applied and then silently overwritten by the stale server value — the rule CLAUDE.md draws from the dyn-settings incident that cost two wrong deploys. ⚠ The push is raced against a 6s timeout because navigator.onLine reports "online" in aeroplane mode in the webview, as signOut() below already documents; a timeout leaves the entry QUEUED, since "we do not know" is not success.
