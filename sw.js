/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v655';   // v655: THE AUTH TRACE, PASS TWO -- and pass one KILLED MY OWN HYPOTHESIS, which is the point of having shipped a measurement instead of a fix. The owner's trace shows signOut:late resolving 2ms after SIGNED_OUT and ~10 SECONDS BEFORE the next sign-in, so the uncancelled Promise.race loser clobbers nothing and the signOut race is DEAD as a suspect. ⭐ TWO BETTER FINDINGS REPLACED IT. (1) The account hash is IDENTICAL either side of the failure and he did not force-close between, so this is NOT about switching Google accounts at all -- it is sign out then sign in again in the same app session, which is far easier to reproduce. (2) The failure is exactly located: TWO deepLink events, the first followed by no authChange, no writeIdentity and no session whatsoever, the second producing SIGNED_IN 2.3s later. ⚠ It is INTERMITTENT -- a later attempt succeeded first time -- which is consistent with a race and is why it needed several tries in quick succession. ⛔ WHY IT IS SILENT: all THREE exits of handleAuthDeepLink end in console.error/console.warn, and neither the Android WebView nor the iPhone PWA has a console, so a failed sign-in looked like nothing happening. WHAT PASS TWO ADDS: the deepLink mark now reports WHICH branch ran (tokens / code / error / none), whether supabase's PKCE code verifier is still on disk, and the OUTCOME of setSession and exchangeCodeForSession including the error text. ⚠⚠ THE VERIFIER IS ASKED ABOUT FOR A REASON: supabase stores it as `sb-<ref>-auth-token-code-verifier`, and THAT KEY MATCHES signOut()'s forceLocal filter (sb- prefix AND contains -auth-token), so a sign-out deletes the verifier along with the session. If the failing deep link reports verifier=n and the working one verifier=y, that is the bug. ⛔ STILL NO FIX -- one wrong hypothesis has already been retired by data and I am not replacing it with a second guess in the file where a mistake means people silently stop staying logged in. ⚠ NO TOKENS AND NO CALLBACK URL ARE RECORDED (the URL carries the tokens); only the branch name and supabase's own error text. All five auth harnesses pass unchanged. auth.js is precached. v654 is a peer's and is spent.
