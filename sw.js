/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v652';   // v652: THE OFFLINE CTA NAMES THE QUIZZES, and the last meta descriptions that still described a site without them. (1) app-cta.js SURFACE[] is the card's FIRST sentence (it renders SURFACE + ' ' + PLATFORMS), so only that map moved and the platform line is untouched -- owner's wording: "Download this topic and its quizzes to listen with no internet", with index and playlist following. ⛔ `read` IS DELIBERATELY UNCHANGED: the reading course has TESTS, not quizzes, and the two words were separated on purpose in v644 so one of them stops meaning two things. (2) Meta descriptions: the home page (which lives in gen_home_splash.js, since index.html is GENERATED), subscribe, join and start. Audited all 18 top-level pages; read.html is left alone for the tests-not-quizzes reason, and the legal/social pages have nothing to claim. ⚠ playlists.html, progress.html and account.html have NO meta description at all -- a pre-existing gap the quiz launch did not create, logged rather than fixed here. (3) ⚠ FIXED A LATENT SPLIT FROM v644: app.html's <meta name=description> was updated then but its og: and twitter: mirrors were NOT, so the page had been advertising two different descriptions for eight versions. gen_static_seo.js mirrors all three from the head, so re-running it resynced them -- the lesson being that editing a description by hand touches ONE of three tags and the generator is what keeps them in step. (4) The subtitle gap on the 68 units that have one: .topic-subtitle margin-bottom 10px -> 4px, so the "N sentences ... Take the quiz" line sits under the subtitle instead of floating below it (owner: "when they do have a subtitle, that space is slightly too big"). ⚠ THAT EDIT IS NOT IN THIS COMMIT -- it was swept into the peer's v651 by a `git commit --only`, which commits a path's state ON DISK including another session's edits. Recorded here because v651's message does not mention it and a later reader would not find it. app-cta.js, index.html, join.html, subscribe.html and app.html are precached and cache-first. v651 is a peer's and is spent.
