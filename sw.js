/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v555';   // v555: THE มา UNIT GAINS THE HORTATIVE, AND ONE GLOSS CHIP IS CORRECTED (owner, 2026-09-18). (1) GRAMMAR-02 9 -> 10 SENTENCES. #2600 มาเป็นแฟนกันไหมครับ is BORROWED from Romance2_LI1_S1586 (§15): same text, its OWN global number, its own R2 file, its own play counter -- reusing #1586 would have merged its plays into the topic original's, because the counter keys on `num` alone. The unit taught five uses of มา and none of them was the invitation (มา + verb + กัน, 'come on, let's'), which is the one use that is neither a direction nor a stretch of time. NO STUDIO TIME: the Andovar take already exists under the topic name and is COPIED to the Gram filename -- one recording, two files. Cultural note re-authored for มา (a borrow arrives written for its source topic). (2) #283 (Food & drink 2) glossed มา as '(ended up)' while its own English reads 'I ordered too much food' -- a chip claiming a word the translation does not contain. It is 1 of 104 มา chips in the corpus to say that; the convention for V+มา is '(bring back)' / 'brought' / '(and came back)'. Now '(and it came)'. ⚠⚠ ORDER TRAP FOUND HERE, AND IT IS SILENT: gen_audio_versions.py fingerprints the clip set named by topic-sentences.json, NOT whatever is on R2 under the prefix. Run it before gen_topic_sentences.js and a newly added sentence is INVISIBLE to the stamp -- it reported 'no change' for GramMaa with the new clip already uploaded. The order is gen_topic_sentences.js -> upload -> gen_audio_versions.py -> refresh_legacy_stamp.py. BOTH stamp schemes moved for GramMaa_LI1, which is what actually reaches a device holding the download: the legacy key is the only channel that can reach a device still on a legacy baseline. ⚠ Two UNRELATED stamps were found drifting and are corrected here (CommSurvival_BEG, GettingToKnow_BEG) -- their R2 audio has not matched the published fingerprint since the Andovar switchover, so those downloads were never offered the update. ⛔ Meditation_LI2 is DELIBERATELY HELD at its committed value: v541 pulled S1312 and states plainly that audio-versions.json was left untouched on purpose, since the sentence is parked and returns with the pickup. Its fingerprint moves because the NEEDED SET shrank, not because any audio changed -- shipping it would spend a re-download round v541 chose not to spend. Expect gen_audio_versions.py to keep reporting it until S1312 is back. // v554: THE SITE COPY CATCHES UP WITH THE ANDOVAR SWITCHOVER (owner-approved, 2026-09-18). The Thai became real studio recordings on 2026-09-15 and no user-facing string said so for three days. ⛔⛔ AND EVERY SPEED CLAIM CAME OUT -- 'native-speed' / 'natural-speed' appeared in 111 places and is now in ZERO. Owner's reasoning, and it is the standing rule now: a speed claim is read as a PROMISE ABOUT PACE, so the learner arrives expecting sentences pitched at some appropriate speed and we have set them up to be disappointed. The recording claim is the one to lean on. ⚠ DO NOT REINTRODUCE A SPEED CLAIM IN MARKETING COPY, in either wording -- it was banned in AD copy separately (ADS_OPERATIONS.md decision 16) and is now off the site too. The dyn player's Thai-speed SLIDER is a different thing and may be described as a control (about.html and guide.html now do). ⚠⚠ THE CLAIM NAMES THAI, ALWAYS. Only the Thai is human; the English translations are still Chirp3 (en-US-Standard-D), so a bare 'recorded by native speakers' over-claims by half the product. Every string written today says Thai sentences / native Thai speakers / Real Thai voices. Changed: the home splash tile + its meta/Organization schema (gen_home_splash.js and home-mock.html, which is the DESIGN SOURCE and is gitignored in /live -- edit it there or the next generator run reverts you); 93 topic heads + Course schema (ssrify_topic.js -> gen_og.js); 92 topic intros (topic-intros-draft.txt -> gen_intros.js, plus the topic-data copies so a later sync cannot resurrect the old phrase); /topics and the 5 band pages (gen_topics_pages.js); 15 Read Thai heads (ssrify_read.js -> gen_static_seo.js) and read.html + read-quiz.html by hand; about, subscribe, app, guide, manifest.json, llms.txt, grammar.html, og-image.png. ⚠ /start is deliberately untouched (owner: it is an ad landing page) and had no speed claim anyway. The bump is load-bearing: index.html, about.html, guide.html, app.html, subscribe.html, read.html, manifest.json and the band pages are all precached and served cache-first since v292, so without it a returning device keeps the old copy indefinitely. // v553: THE READING SECTION'S TWO DOWNLOAD DIALOGS. (1) "Remove the offline copy of the Read Thai course?" was still a window.confirm -- the plain grey browser box, next to a card styled like the rest of the site (owner, 2026-09-18). It now uses the .te-modal idiom every other destructive confirm on the site uses (progress.html's reset, read.js's own "Clear my reading progress", and by family the account page's "Remove ALL downloads?" and the topic bar's "Delete this download?"). No typed confirmation: this removes ONE download rebuildable in a tap, not every download on the device. (2) THERE WAS NO LEAVE GUARD AT ALL while the 208 clips download -- a tap on the nav silently abandoned it, where a topic download has warned since the dyn rollout. player.js's installLeaveGuard is duplicated into read.js: a capture-phase click listener answers an INTERNAL LINK with the site's own dialog, and beforeunload stays as the backstop for tab close / the URL bar. ⚠ beforeunload's text CANNOT be styled or reworded -- browsers have ignored author-supplied wording since ~2016 -- so intercepting the link click is the only route to a dialog that says what is at stake. ⚠ read.js's window.confirm survived the 2026-08-27 sweep because the comment left behind claimed that pass had removed the LAST one on the site; it had not, and a comment asserting a site-wide fact is exactly the thing that goes stale unseen. Both callers now share one readModal(). ⚠ The two dialogs differ in which button is filled: the remove confirm fills the DESTRUCTIVE one (red, .te-modal-confirm), the leave guard fills the SAFE one (accent, the new .te-modal-primary -- "Keep downloading" against a quiet "Leave anyway"), mirroring player.js's dynConfirm. read.css + read.js are precached and served cache-first since v292, hence the bump. // v552: TWO VOWEL NOTATIONS, ONE PER TEXT ENGINE, each the version already PROVEN on that engine (owner's call after v551). WebKit (iOS PWA + Safari) gets the red อ carrier with ◌ as the trailing slot; Blink (Android app + desktop) gets dotted circles throughout. ⛔ THREE PORTABLE ATTEMPTS FAILED, DO NOT RETRY ANY: circles everywhere (WebKit draws a SECOND circle for the mark); อ + red glyph (Blink paints the whole grapheme cluster red, the Android WebView loses the colour entirely); อ + red underline (cuts through below-vowels อู / อุ). ⚠ THE PROBE MEASURES THE DEFECT: does U+25CC + mark take ONE glyph of width or TWO? Blink 0px gap, owner's iPhone 20.31px across two loads. ABSOLUTE gap, never a ratio -- Sarabun has no U+25CC glyph so the circle is fallback-resolved and its size changed between those loads. ⛔ Never sniff the UA; never probe an orphan mark's width (that was v546 and it regressed iOS -- a non-zero advance is not a drawn circle). ONE SOURCE: the data holds the อ form and toCircles() derives the other by swapping the FIRST อ of each form, which is always the placeholder. ⛔ The DESCRIPTION switches with it, swapped as a whole sentence pair. Wired into every surface that draws or names a symbol -- cards, alt forms, BOTH quiz modes, the tone-mark row, the tone-table headers (needles track the notation), the silent-letter heading, the intros, the grid note and the ⓘ -- because markup is per-site even though the data is shared. Consonants untouched on both branches. SSR bakes the circle form with no special-casing: no layout engine means the probe measures 0 and falls to Blink, which is what crawlers and most visitors get anyway. // v551: the placeholder อ is marked with a red UNDERLINE, not a red glyph. v550 used `color` and broke differently on every engine (owner, all three on v550): desktop Blink turned the WHOLE of อี red including the mark, the Android app showed no red at all, the iPhone PWA was exactly right. ⛔ CAUSE: อ AND ITS COMBINING MARK ARE ONE GRAPHEME CLUSTER, painted as a unit even though the mark sits OUTSIDE the span. Blink paints it with the span's style so the red bled onto the mark; the Android WebView paints it with the parent's so the red vanished; WebKit splits the cluster at the element boundary -- the same behaviour that caused the original doubled-circle bug -- so it alone looked right. NO color RULE CAN BE CORRECT ON ALL THREE. A border paints on the element's OWN BOX and cannot bleed; the mark has zero advance so the box is exactly the อ (measured 26.1px against a 165.4px glyph) and every character stays plain black. ⚠ Do not return to color for this and do not fork it per platform -- the same cluster-painting rule will break it again the same way. // v550: THE PLACEHOLDER อ IS RED, a real อ stays black (owner, 2026-09-17). Since the carrier became อ, a symbol like ออ or อ็อ◌ shows อ twice for two different reasons -- once as the slot a consonant goes into, once as a letter genuinely spelled in the word (ขอ, ล็อก). ⭐ THE RULE: THE FIRST อ IN EACH FORM IS THE PLACEHOLDER, every real one comes after it. Checked against all 26 symbols, no exceptions. ⚠ IT IS A RENDER-TIME WRAP, NOT DATA, so unlike the character change it does NOT come along for free -- every site that draws a symbol had to be wired: the cards, the alt forms, BOTH quiz modes and the tone-mark row. The owner asked specifically whether the quiz would follow; it does not by itself, and that is the difference between sharing DATA and sharing MARKUP. ⛔ VOWELS ONLY. The first pass wired it unconditionally and would have painted the CONSONANT อ (aw aang) red on read-mid -- a real letter with its own card. Caught because read-mid.html came back changed when it had no business changing; the render sites now gate on the existing  flag. ⚠ #C0392B is also the quiz's wrong-answer red, but that state colours a tile's border and background and never glyph text, so they do not collide; --ph-color is one variable if it ever needs to. // v549: fixes one sentence v548 broke. The vowels intro explains that a ◌ AFTER a form marks a following final consonant -- and the prose pass that swapped carriers to อ rewrote the ◌ inside that very explanation, so it read 'where a second form has a อ after it'. ⚠ A blanket token replacement will happily edit the sentence that DOCUMENTS the thing being replaced; the two circles left on the page are trailing slots and are correct. Found by reading the rendered page rather than the diff. // v548: Thai combining marks are carried on อ instead of the dotted circle. WebKit/CoreText will not attach a mark to U+25CC -- it lays out a SECOND dotted circle to carry it, so ◌ี read as two circles on the iPhone PWA while Android was correct. ⛔ CHANGED IN THE DATA, ONCE, FOR EVERY PLATFORM -- no runtime branch and no engine detection. Two attempts to detect the engine failed and the second shipped a regression (v546); the point of doing it this way is that there is nothing left to get wrong. ⚠ ONLY the circles that actually CARRY a mark changed, 19 of them. A dotted circle with no mark shapes correctly everywhere -- which is why sara o's ◌◌ was right on iOS all along -- so every final-consonant slot and every เ◌ / โ◌ / ◌า keeps its circle: อั◌, not อัอ. 44 dotted circles remain. Stacked อ would read as a syllable. อ is the conventional carrier in Thai teaching (อา, อี, เอ), so it is a notation the reader already knows. The vowels intro now says 'The อ or ◌ in each symbol', since both appear. // v547: REVERTS v546 ENTIRELY. The per-engine dotted-circle strip was WRONG and made iOS worse, not better: sara ii, sara uu and the first form of sara uue lost their circle completely, and in เ◌ิ◌ the ิ attached itself to the preceding เ instead. ⛔ THE FALSE PREMISE, recorded so nobody retries it: v546 assumed CoreText DRAWS ITS OWN DOTTED CIRCLE for an orphan Thai mark, and probed for it by measuring whether U+0E31 alone has width. It does have width on WebKit -- but a NON-ZERO ADVANCE IS NOT A DRAWN CIRCLE. CoreText gives the orphan mark an advance and then attaches it to whatever precedes, or leaves it floating. So the probe answered a different question from the one that mattered, and removing our U+25CC left the marks with no base at all. ⚠ Measuring the right THING is not the same as measuring the right WAY -- the probe was moved from canvas to a real laid-out box, which made it more faithful to a question that was already the wrong one. ⛔ Do not re-attempt this by measuring advances. The original artefact (CoreText adding a circle NEXT TO ours, so ◌ั reads as two) is still open and needs a different remedy -- a real base character, or accepting it. // v546: THE EXTRA DOTTED CIRCLE ON EVERY THAI VOWEL SYMBOL, on the iPhone PWA only (owner, 2026-09-17; the Android app was correct on the identical build). ⛔ THE U+25CC PLACEHOLDER IS NOT PORTABLE AND THE TWO ENGINES FAIL OPPOSITE WAYS. WebKit/CoreText will not accept U+25CC as a base for a Thai combining mark -- it inserts its OWN dotted circle to carry the mark, so ◌ั drew as two circles with the mark on the SECOND and ◌ั◌ as three. Blink does the exact reverse: an orphan Thai mark gets ZERO advance and no circle at all, so on Android and desktop the placeholder is the only thing making the mark visible. No single string is right on both, so the engine is measured once at boot and the matching string emitted -- strip our placeholder where the engine supplies one, keep it where it does not. It is self-correcting by construction: exactly one circle either way. ⚠ MEASURE THE ENGINE, NEVER SNIFF THE UA -- this is shaping behaviour, not browser identity. The probe renders U+0E31 alone in a hidden span and reads getBoundingClientRect(), i.e. a REAL LAID-OUT BOX rather than canvas measureText, because canvas does not always run the identical shaping path and this probe is the whole fix. It is font-independent, so it need not wait for Sarabun. Applied inside esc(), which every rendered string already passes through, so it covers the vowel symbols, the alt forms, the four tone marks and the intro prose that names them. ⚠ Blink is byte-identical to v545 -- verified, probe reads 0 and the strings are unchanged. The iOS half CANNOT be verified from this machine: no engine here reproduces the artefact. v545's sarabun-thai-500 preload stays -- it was a real gap, but it was never this bug. // v545: the 15 reading pages also preload sarabun-thai-500. Every Sarabun face is font-display:optional, which grants ~100ms and then falls back FOR THE WHOLE PAGE LOAD rather than swapping in later, and only weight 400 was preloaded -- while the reading course draws its big glyphs at weight 500 (.lc-ch, .tc-thai, .tq-big-symbol, .c-sym, .qc-thai). A fallback shaper will not attach a Thai vowel mark to U+25CC; it emits its own dotted circle to carry the mark, so a two-slot form renders as THREE circles with the mark on the second, which is both the artefact the owner reported and the extra width making multi-form cards cramped. ⛔ READING PAGES ONLY: the other 143 pages use Thai at weight 400, already preloaded, so site-wide would be 9.7 KB of cost for no benefit. ⚠ THIS ONLY FIXES THE FIRST-VISIT CASE. All four Sarabun weights are ALREADY in PRECACHE, so once a worker has installed the font is served from cache and never races at all -- a returning device showing the artefact is NOT explained by this, and the remaining suspect is the iOS/CoreText shaper, which refuses U+25CC as a base for Thai marks regardless of which font is loaded. ⛔ Do NOT 'fix' that by writing the mark bare: Chrome renders an orphan Thai mark at ZERO width with no dotted circle at all (measured), so dropping U+25CC trades an iOS artefact for an invisible vowel everywhere else. font-display:swap would also close it and is deliberately not taken -- optional is there to stop the flash and the layout shift. // v544: THE MULTI-FORM VOWEL GLYPHS SPILLED OUT OF THEIR CARD AT LARGE OS TEXT SIZES -- 81px past a 150px card at 2x, and 51px past a 180px card. altHtml() emitted ONE nowrap span for the whole alt string, so '/ เ◌ิ◌ / เ◌ย' was a single unbreakable unit with nowhere to wrap. It now emits ONE SPAN PER FORM: each form is still nowrap, so a vowel symbol never splits down the middle, but a line break is allowed BETWEEN forms. ⚠ INVISIBLE AT 1x -- every card fitted at the default size, which is why v543 shipped it. iOS -webkit-text-size-adjust scales px text, so this reached any iPhone with Dynamic Type turned up (TEXT_SCALING.md §3); Android textZoom leaves px alone and would never have shown it. Measured after the fix: ZERO overflow on cards, quiz tiles and the big prompt at 1x, 1.5x and 2x, at both the 150px mobile grid minimum and 180px. ⚠ 3x still spills ~24px and is left that way ON PURPOSE: the only way to fix it is to let a break fall INSIDE a form, which would split a vowel symbol across two lines and teach nonsense. 2.0 is the supported ceiling anyway -- it is what applyIosTextScale clamps to and what WCAG 1.4.4 asks for. // v543: sara uhr now shows ALL THREE of its written forms on the card AND in the quiz, and its info note is gone (owner, 2026-09-17). The third form is the COMMONEST -- 354 corpus occurrences against 222 and 207 -- so putting it behind a tap was backwards. ⛔ EVERY FORM ON A MULTI-FORM CARD IS NOW THE SAME SIZE: the container carries .has-alt and steps down, rather than the part after the slash being shrunk. Uniformity ACROSS THE SLASH reads as one fact about one vowel; two sizes read as a main form plus a footnote. A multi-form card is therefore SMALLER than a single-form one (22px against 44px), which is deliberate and owner-approved. ⚠ The sizes are MEASURED, not chosen: the widest string (เ◌อ / เ◌ิ◌ / เ◌ย) caps at 23px in a 180px card and 19px at the 150px mobile grid minimum, so 22/18 sit just inside with every card fitting on one line. Re-measure before changing a glyph string -- adding a form shrinks the ceiling for all of them. // v542: THE READING COURSE NOW SHOWS THE SECOND WRITTEN FORM of the 8 vowels that change shape before a final consonant (Thai: สระเปลี่ยนรูป / สระลดรูป). Owner, 2026-09-17: the course taught one form each while its OWN examples already used the others -- ฟัน and รัก for สระอะ, เด็ก for สระเอะ, สวย as the worked example for the -y final, and ธง as the exemplar word for ธ in the very first low-class step. A learner met the changed shape in step 1 and was never told it was a vowel. Cards and quiz tiles now read "เ◌อ / เ◌ิ◌". ⚠ A NEW `alt` FIELD, NEVER A CHANGE TO `ch`: read.js keys letterAudioMap() and clsOf on `ch`, and the vowel quiz uses it as both the prompt symbol and the choice tiles, so putting a slashed pair in `ch` would quiz a string rather than a vowel. altHtml() is the single renderer for all three sites. ⚠ NO NEW AUDIO, and that is the point -- the two forms sound identical, so this costs nothing now the 210 reading clips are Andovar studio recordings and r2_upload_read.py refuses to revert any of them to TTS. AUDIO_VER is deliberately NOT bumped: no clip moved. Two vowels carry an ⓘ note, reusing the existing .gl-term/.gl-pop glossary popup rather than adding a second tooltip -- openPop() falls back to inline data-title/data-def when the key is not in GLOSSARY. สระโอะ needs one because its reduced form is NOTHING AT ALL (คน, รถ, ธง are written with no vowel), and สระเออ needs one because it has a THIRD form, เ◌ย before a final ย -- which the corpus says is the commonest of the three (354 occurrences against 222 and 207, driven by เลย at 319 and เคย at 24), but three glyphs will not fit a two-column quiz tile so it lives in the note. ⚠ The ⓘ sits inside a card that is itself a play button, so wireGrid stops propagation on it or tapping the note would fire the audio. ⚠ .lc-alt is sized in `em`, not px: iOS -webkit-text-size-adjust scales px text too (TEXT_SCALING.md §3), so an em keeps the alt in proportion to the glyph at every setting instead of overtaking it. The containers wrap rather than overflow -- at 2x a 150px card cannot hold both forms on one line, and a second line is better than spilling out of the box. // v541: S1312 (ปฐมฌาน, Buddhism 7) is PULLED, temporarily and without touching the Thai. The Andovar male take reads the FIRST ฌาน as ญาณ -- pathom-YAAN where the authored translit says pathom-CHAAN -- while the second occurrence in the same line, ruup-pa-chaan, is correct, so it is a reader substituting the commoner word and not a text fault. Pulled rather than left live because every unit is dyn: ordinary playback STITCHES the per-sentence clips, so the faulty take would be baked into every built session and every offline download, and the pill would go on offering it. TTS is not an option either -- it would put a synthetic clip in among 2,458 studio recordings. The sentence is parked WHOLE in topic-data/_parked/topic-35d_S1312.json and comes back with the pickup re-record; audio-versions.json and the R2 clips are deliberately untouched, so the restore is a paste and a regen with no upload. THE BUMP IS LOAD-BEARING: sentence-hints.json and topic-sentences.json are both in PRECACHE and both lost the entry, and precached sub-resources are served cache-first since v292. Buddhism 7 is 10 sentences -> 9 (topics.js, the head count and the card numbers all follow). ⚠ TE/ET for Meditation_LI2 still CONTAINS the faulty line -- it is prefab and only ever plays as the prev/next placeholder, from sentence 1, so rebuilding it was judged not worth an upload round for a sentence that is coming back. // v541b: S1286 (Buddhism 6) glosses the discontinuous frame ยึด...เป็น as ONE chip, 'hold as', per the convention the corpus already uses in 7 places (ASCII dots in the Thai, a spaced ellipsis in the translit). 'hold to' sat one synonym from ยึดมั่นถือมั่น 'you cling' in the SAME unit, which read the Triple Gem sentence as being about attachment -- the opposite of what it says. // v540: AUDIO_VER 4 -> 5, for one clip. ก ไก่ (read/gaw-gai.mp3) had a 0.577 s pause between "gaw" and "gài" -- measured across all 45 consonant clips that is 2.5x the next longest (0.228) and 5.5x the median (0.104), so the owner's ear was reading a real outlier, not a preference. Fixed IN HOUSE rather than as a pickup: 0.427 s of room tone removed from the MIDDLE of the pause, leaving the tail of "gaw" and the attack of "gài" untouched, so it is an edit to the delivered take rather than a new one. The join sits in silence -- sample step 0.0003 against a 0.168 peak elsewhere in the clip -- so there is no click. The 1.0 s tail was checked and LEFT: the median tail across the 45 is 1.034 s, so it is normal. ⚠ A single-clip fix still costs a whole-course AUDIO_VER bump, because versioning is per-course and not per-clip; that is the only mechanism there is, and leaving it unbumped means every device keeps the old bytes (Cloudflare keys on the query string) and is never offered the update. // v539: the Muay Thai illustration is redrawn with TRADITIONAL HAND WRAPS in place of modern gloves (owner, 2026-09-16). A bump is load-bearing and not tidiness: muaythai.png is in PRECACHE and precached sub-resources are served CACHE-FIRST since v292, so replacing the file without one leaves every returning device on the old art INDEFINITELY. ⚠ The new asset is letterboxed onto the live 185x317 canvas rather than shipped at its own aspect (613x1085 = 0.560 vs 0.584). .fluency-img sets width:158px/height:auto, so the file's intrinsic ratio decides the rendered height, and topics.html's width="150" height="257" is that same ratio reserving the box before decode -- native aspect would have rendered ~11px taller than the reservation and reintroduced the layout shift those attributes exist to prevent. The ~8px of transparent side padding is invisible; a shift is not. 15.5 KB against the old 16.3 KB, so the re-compression is genuinely smaller as the precache rule requires. // v538: TWO LIVE READING-SECTION FAULTS. (1) AUDIO_VER 3 -> 4. All 210 reading clips became Andovar studio recordings on 2026-09-15 and NOBODY was told: dlDownload stores AUDIO_VER in its manifest and the hub card offers an update only when they differ, so the new audio shipped while devices kept serving cached TTS. It also fixes the edge -- an upload purges read/x.mp3 but Cloudflare keys on the query string, so read/x.mp3?v=3 kept the old bytes; a new query string misses every cache at once. ⛔ BUMP THIS ON EVERY READING-AUDIO RELEASE. (2) A fetch that never settles froze the whole course download ONE SHORT -- the pump loop does active--/pump() on every path, but a request that neither resolves nor rejects reaches none of them, so the counter stopped at 207 of 208 for ever with no error. Every clip was on R2 and returned a clean 200 when probed, which ruled the server out. dlFetchWithRetry races a 20s backstop, aborts the socket and retries 3x -- the same guard player.js's dynDlFile has had since r44 and this path never got. // v537: the guide's Thai-speed cycler line is the owner's wording -- "Didn't quite catch that? Slow down the Thai. English sentences and pause length are unaffected." The draft said "without changing its pitch", which is how the thing WORKS rather than what it is FOR: pitch preservation is the implementation's problem, not the reader's. Same class as the app-CTA copy rule -- describe the benefit, not the mechanism. // v536: Thai speed now governs an INDIVIDUAL SENTENCE TAP as well, and the guide's mock player gained the slider. A pill is the one surface where a plain <audio> playbackRate is exactly right -- the clip is the Thai ALONE, so there is no English and no pause to drag along, which is the very constraint that forced the dyn session to bake the stretch in at build time -- and it is instant, no rebuild. preservesPitch is set explicitly in all three spellings: modern browsers default it true but older WebKit honours only the prefixed name, and without it a slowed pill would drop in pitch while the same sentence in the session did not. ⚠ It applies WITHOUT waiting for a rebuild, which the owner flagged as a risk: the slider assigns dynSpeed synchronously and before dynSaveSettings/dynInvalidate, and the only other writers are boot/account-sync and dynRevertToStored (which runs only inside a build-failure catch). Pinned by test_dyn_speed.js, now 42. The old per-pill tortoise stays as an unreachable override -- it is display:none on every live page. // v535: THE THAI-SPEED STRETCH IS NOW PITCH-SYNCHRONOUS, because a FIXED hop was audibly worse on one voice: "sounds a bit robotic at 80%... not really noticeable on female voice" (owner, 2026-09-16). Measured, it is not timbre but PITCH -- a 256-sample hop is 93.8 Hz, our male talent sits at 139-162 Hz and the female at 222-229, so the artefact period sat right next to a male voice's own pitch and beat against it while a female voice masked it. The search was lopsided too: +/-256 gave a male voice only +/-1.5 pitch periods to choose a join from against +/-2.4 for a female. Every size is now derived from the clip's OWN period (hop 2P, window 4P, search 1.5P, overlap 2P), so joins are period-ALIGNED by construction and the hop rate moves away from f0. ⛔ dynPeriod() must not be "optimised": decimating it returned 170/200/126 and an unbiased NCC returned 146/190/120 where this returns 148/173/105 -- the values the approved renders were built from, and every window is a multiple of that number. Affordable because it is CACHED PER CLIP (~2.3 s once per 30-clip topic, only when the speed is not Normal). A sub-harmonic guard stops it picking an octave on cleanly periodic audio without moving any real clip's answer. test_dyn_speed.js 38. // v534: THAI SPEED, a per-unit dyn setting alongside Pauses and repeats (owner, 2026-09-16). ⛔ IT SLOWS THE THAI AND NOTHING ELSE -- never the English, never the silences -- which is WHY it cannot be an <audio> playbackRate: the dyn player plays ONE stitched file, so a rate on the element would drag the English and every pause with it. The stretch is applied per Thai buffer at BUILD time by dynStretch(), a WSOLA that overlaps and adds short windows so the pitch is untouched and only the timing moves (resampling drops the pitch with the rate -- the owner listened and rejected it outright). Steps are 1 / 0.9 / 0.8 and the slider shows no number: 0.7 and 0.75 were auditioned and rejected as smeared, so 0.8 is the floor and dynNormSet refuses anything below it. Speed joins the session key ONLY when it is not 1, so every session persisted before today stays valid. ⚠ The stretch reads st.sp, never the page-global dynSpeed -- this function builds FOREIGN units too (the prev/next chain renders a neighbour at ITS settings), and reading the global is the r214 namespace bug's shape. test_dyn_speed.js 32, and it goes red on the pre-fix source. // v533: THE UPDATE BAR STILL FLICKERED ON ANDROID after v532 -- and NOT on the iPhone PWA, on a device confirmed to be running v532 (owner, 2026-09-16). v532 guarded the one direct bar.innerHTML writer; whatever repaints on the app path is not it, and rather than guess again the hold is now UNCONDITIONAL. offBarLock is taken the moment Update is pressed and setOfflineState() -- the single painter for every state that carries a button -- refuses to paint anything but 'downloading' while it is held. That covers every caller at once instead of one route at a time. Released in FOUR places so it can never strand the bar: both of dynDownloadHere's never-started exits, its success handler and its catch. downloadingNow was the wrong thing to guard on -- it is read in half a dozen places and cleared on every exit of three download paths, so trusting it means trusting all of them. ⚠ STILL UNDIAGNOSED: what repaints on Android and not on WebKit. If it recurs, ?lat=1 reports offBarHeld (repaints suppressed) in dl:done. // v532: THE UPDATE BAR FLICKERED BACK TO ITS BUTTONS MID-DOWNLOAD, so Update could be pressed twice (owner, 2026-09-16, Android app). dynCheckAudioUpdate() paints bar.innerHTML DIRECTLY from an async .then(), so it answers to neither setOfflineState()'s idempotence guard nor renderOfflineBar()'s downloadingNow hold -- a check that started before the tap landed after it and painted Update/Delete over "5 of 50". It now stands down while a download is running. dynUpdateAudio() also LATCHES downloadingNow and paints "Downloading 0/?" the instant the button is pressed, so the progress line is up before any async work and a second press is impossible rather than merely unlikely. Both of dynDownloadHere's never-started exits (offline, nothing to fetch) now clear the latch, or they would freeze the bar at 0/? -- worse than the flicker. Same class as r210, via the one path that bypasses the shared painter. // v531: A FAILED AUDIO UPDATE COULD NOT BE RETRIED. The v530 skip left the old clips in place, and the finalize block then stamped the CURRENT audio version onto the baseline regardless -- so afterwards no surface offered the update again and only delete-and-redownload recovered it (owner, 2026-09-16, two topics stuck showing a tick). The stamp now runs only on a first download, on a prefix that was genuinely re-fetched, or when the baseline did not say anything had moved anyway; otherwise the baseline is left alone and keeps prompting. A repeated prompt is recoverable, a false "you are current" is not. // v530: THE AUDIO UPDATE PROMPT AND THE DOWNLOAD THAT ANSWERS IT ASKED DIFFERENT QUESTIONS. Owner, 2026-09-16, after the Andovar switchover: the topic page offered "Download audio update?", the button completed instantly, and the topic went on playing the superseded TTS -- while the card in the list still showed a plain tick. One cause for all three. avPick() always prefers the clip-derived `#c` key, so a device whose baseline is still legacy-scheme got a `#c` value back and avMoved() correctly read the SCHEME change as "not moved". dynCheckAudioUpdate happened to compare against the raw legacy key (map[pfx]) and so DID prompt; dynDownloadHere's force check and topics-page.js's avStale both used avPick and so did not, which is why the prompt was real and the download skipped every file as "already present" -- the r137 defect in a new disguise, reached by a different route. topics.js gains avFor(map, prefix, base): the published value in the BASELINE's own scheme, falling back to avPick when nothing in that scheme is published, so the deliberate silent re-baseline survives for the case it was built for. All four readers now use it -- player.js (prompt AND force), topics-page.js (topic cards, band pages, favourites and the grammar hub) and pl-list.js (playlist rows). Paired with the legacy keys moving for the first time since Phase 4 (refresh_legacy_stamp.py), so both generations of device are told. // v529: app-cta.js stops emitting bare `.html` links. Its three
