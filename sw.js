/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v645';   // v645: THE TOPIC CARD NOW REPORTS A SUPERSEDED PAGE, NOT JUST SUPERSEDED AUDIO. Owner, 2026-09-23, minutes after v643: "the ticks are still showing for all - page update available message isnt triggering the appearance of dotted circles". v643 taught the topic PAGE to notice its download was out of date and taught the card NOTHING: avStale() only ever compared audio-versions.json against the recorded `av`, so the page offered an update while the card beside it said "downloaded". ⚠⚠ THIS IS THE SAME DIVERGENCE topics-page.js's own avFor() note records from 2026-09-16 -- "the card kept showing the TICK while the topic page itself offered the update" -- reintroduced on a NEW AXIS the moment one was added. A surface gaining a question the others do not have is the recurring fault on this codebase, not a one-off. ⚠ WHAT THE CARD CAN SEE, AND WHAT IT CANNOT: player.js compares contentHash(), the page's sentences AND quiz block, but it is ON that page; a card cannot hash a document it does not have. It reads the manifest plus the PUBLISHED per-unit quiz stamp (quiz-data/index.json `sig`, written by gen_quiz_data.js beside each side-car), which covers an answer-key change and a C3 prompt rewrite because `orders` and `enq` both live in the side-car. ⛔ It does NOT cover a pure sentence-TEXT edit, which moves contentHash but not the side-car: the page catches that, the card does not. Under-reporting, never over-, and documented rather than papered over. TWO ABSENT-BASELINE FLIPS, both one-time and both self-clearing: no `ver` (predates v643) and no `qz` (updated on v643/v644). The second matters more than it looks -- left out, every device that updated in that window would be permanently blind to the next answer-key change while the page kept contradicting it, i.e. this exact bug one release later. ⛔ Adopting the published sig instead would be worse: we cannot know a downloaded side-car matches it, and adopting would silently declare a stale download current. player.js's download finalize now records `qz` beside `ver`, which is what clears both flips -- safe on the prefix-keyed manifest because prefix<->unit is 1:1 (MEASURED: 113 units, 113 distinct audio prefixes; a split topic's parts do NOT share one, contrary to a comment elsewhere saying they do). ✅ 6 new assertions in test_topics_page.js, 3 red on the shipped source including the owner's exact report; the fetch stub had to be taught to ANSWER quiz-data/index.json first, because contentStale() returns false without it and every one of them would otherwise have passed VACUOUSLY -- which is how the card shipped with no content signal at all. topics-page.js and player.js are precached and cache-first. v644 is a peer's and is spent.
