/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v653';   // v653: A READ-ONLY AUTH TRACE. NO BEHAVIOUR CHANGE. Owner, 2026-09-23: switching Google accounts leaves the app signed OUT after the round trip -- "dropped back to homepage with create an account and log in" -- on BOTH the Android WebView and the iPhone PWA, and a force-close does NOT restore the session, it only makes the NEXT sign-in work. So no session is ever established, and the failure is in ORDERING on a real device under timing this machine cannot reproduce. ⚠⚠ TWO OF MY OWN DEDUCTIONS WERE WRONG BEFORE A LINE WAS WRITTEN: first that storage must be intact (it is not -- force-close leaves you signed out), then that a resume-time reseedSession() would fix it (there is nothing to reseed). Both would have shipped a confident non-fix to the one file where a mistake means people silently stop staying logged in. THAT is why this release measures instead of fixing. WHAT IT ADDS: trace() at six points -- writeIdentity, clearIdentity, onAuthStateChange and its !user-but-signed-in guard, signOut start/late/forceLocal, and the native deep link -- each recording the three facts that must agree (our signed-out marker, our identity record, supabase's own key) plus the event and a 6-char hash of the account. ⛔ Behaviourally inert: every call is a bare trace(...) in a try/catch, no branch, no await, no value anyone reads, and OFF for everyone unless the flag is set by hand. ⚠ NO TOKENS, EMAILS OR user_id are written -- only the hash, because all any line needs is "same account or different" (Golden Rule 0: a truncated id is still personal data). THE SUSPECT IT WILL CONFIRM OR KILL: signOut does Promise.race([attempt, timeout]).then(forceLocal) and a race does NOT cancel the loser, so a slow supabase signOut can still be in flight when the new session is established and complete its teardown afterwards -- which would show as signOut:late AFTER a writeIdentity for a different account. ✅ All five auth harnesses pass unchanged (189 assertions) and the recorder was verified ARMED, because off-by-default means its body would otherwise have run vacuously -- the same trap that let the quiz card ship with no content signal. auth.js is precached and cache-first, hence the bump. v652 is a peer's and is spent.
