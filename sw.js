/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v644';   // v644: THE QUIZ ARM IS PUBLIC, SO THE COPY STOPS PRETENDING IT IS NOT. Owner, 2026-09-23: "all quizzes are now public and live on the site - id like us to change website copy now". Every surface that says what ThaiEar IS now names the quizzes: the home splash's Topics third, the /topics hub, the five band pages, the grammar hub, llms.txt, about, guide, app, join, subscribe, start, and the in-app premium sheet. TWO SURFACES HAD A BULLET NOT EARNING ITS SLOT, so nothing got longer -- subscribe's "Real Thai voices" was no premium differentiator at all (the free topics are native-recorded too, so it was a site-wide truth wearing a premium badge; its claim moved into bullet 1), and the premium sheet promised "every topic" and "all of its audio" as two separate lines. Both consolidated; both freed slots went to the quizzes. ALL 113 UNIT PAGES gained a "Take the quiz" link in the topic-meta line. TAP vs CLICK IS CSS, NOT SCRIPT: both words ship and (hover:hover) and (pointer:fine) hides one -- and the rule lives in the page's OWN style block, not quiz.css, because a stylesheet that has not arrived yet would otherwise render "TapClick to reveal". THE LINK IS STATIC HTML ON PURPOSE: it sits inside a sentence, so revealing it after paint would re-wrap the line and push the player down -- the shift the Progress page was told off for. quiz.js WITHDRAWS it instead if quizGate() ever answers false, so QUIZ_PUBLIC stays the one switch and the copy can never advertise a picker nobody can open. Its href is ?quiz=menu, the url the band-card strip already uses, so entitlement is untouched: the picker opens, the gate fires on choosing a quiz (Decision 1), and the page's own canonical absorbs the duplicate (Decision 5). TWO COPY CLAIMS WERE WRONG AND WERE CAUGHT BEFORE SHIPPING, both the same mistake -- describing HOW a quiz asks rather than what it builds. "Produce the answer rather than recognise it" is false of Vocab Trainer and Listening comprehension, and so is "pick which of three close translations", because both carry a "Hide the answers until I ask" setting. index.html (regenerated from home-mock.html -- it is GENERATED, and a direct edit would have been reverted by the next gen_home_splash run), about, guide, join, subscribe, app, app-cta.js, player.js and quiz.js are ALL precached and served cache-first, so without this bump no returning device sees a word of it. v643 is spent -- a PEER SESSION held it uncommitted on this shared tree for part of this task, then committed and pushed it (bc77f88) while this release was being assembled. v642 IS DELIBERATELY SKIPPED: the peer's own v643 note reserved it for me, its message then agreed v644 instead, and a skipped number costs nothing while a contested one costs a silent regression. ⚠⚠ THE SEQUENCING LESSON, because it nearly shipped two faults. While the peer's bump was still uncommitted, BOTH this file and player.js were staged from the object store (git show HEAD -> edit -> hash-object -w -> update-index) rather than edited on disk, which is the only way to commit your own hunks without taking a peer's. Then their commit landed mid-task and MOVED HEAD -- at which point a blob built from the OLD HEAD would have reverted their fix on commit. Re-derive after any peer commit; a staged blob is a snapshot of a HEAD that may no longer exist.
