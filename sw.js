/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v597';   // v597: the topic-page quiz block is ONE control — all five parts now open the PICKER. quiz.js is precached and cache-first, so this bump delivers it; v596 is spent. The four tiles used to shortcut straight into their own quiz. Owner, 2026-09-21: "i feel like people might just click on it expecting it is one button and the icons are just representations. seems cleaner." ⚠⚠ A block headed "Test yourself on this topic" with a "Choose a quiz" button under it READS AS ONE CONTROL WITH DECORATIVE ICONS, so a tap that landed inside a quiz handed the learner something they had not asked for — the shortcut saved one tap and cost that. The picker is also where the four are actually labelled, described and scored. ⛔ `start` IS NOT RETIRED: ?quiz=vocab still opens that quiz's own menu via bootFromUrl(), because a url naming one quiz is an explicit request for it, unlike a tap on a tile in a block whose own button says "choose". An assertion pins both halves so a later cleanup cannot conflate them. ⚠ The picker's own rows still open the quiz they name.
