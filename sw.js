/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v648';   // v648: THE PAGE AND THE CARD NOW ANSWER THE SAME QUESTION, AND A TABLE HOLDS THEM TO IT. Owner, 2026-09-23: "getting to know you is showing the update circle, but when i go in its showing 'downloaded'". v645 gave the CARD a second flip -- `e.qz == null`, for a download updated on v643/v644 before the quiz stamp was recorded -- and gave the page no counterpart, so the card flagged while the page, whose contentHash matched, called itself current. ⚠⚠ THAT IS THE EXACT DIVERGENCE v645's OWN COMMIT MESSAGE WAS ABOUT, caused by the fix for it. THREE TIMES IN TWO DAYS: v643 the page learned a signal the card did not; v645 the card learned one the page did not; and a THIRD, never user-visible, where the page gated its no-baseline flip on `cfg.quiz` while the card gated it on the published stamp -- so with no readable map the page flagged and the card stayed silent. ⭐ THE THIRD WAS FOUND BY THE NEW TABLE, NOT BY A USER. stale_matrix.js is one list of "is this download superseded?" rows, and test_offline_bar.js asserts them against player.js while test_topics_page.js asserts the SAME rows against topics-page.js. ⛔ NO SINGLE-SURFACE TEST CAN CATCH THIS FAULT: every time, each predicate was individually correct and individually green -- what was wrong was that they answered DIFFERENT QUESTIONS, which only appears when you ask both the same one. ✅ The page's predicate is now literally the card's, plus ONE superset check it alone can make (contentHash covers the sentences too, and requires being on the page). ⚠ The documented asymmetry stays and is deliberately NOT in the table: a pure sentence-TEXT edit moves contentHash but not the quiz side-car, so the page catches it and the card cannot -- under-reporting by the card is accepted, over-reporting would not be. ⛔ THE RULE THIS EARNS: a new staleness signal goes into EVERY surface that reports staleness, in the same change, or into none of them; the axis is irrelevant. player.js is precached and cache-first. v646 and v647 are a peer's and are spent.
