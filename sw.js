/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v724';   // v724: quiz refinement W43 + W43E (QUIZ_REFINEMENT.md W43). The Builder literal lines re-audited with a sense-aware matcher: 748 redundant lines withdrawn, 5 trimmed, 34 kept. 144 published English sentences improved (owner-approved one by one: the missing word put back into the live English instead of a Builder-only line) with new English clips, TE/ET rebuilt around the unchanged human Thai, listening decoys re-authored to parity, hints re-picked, 4 chip glosses. Plus W49/W52 answer-key additions, W47 two-answer 'skill' card, W50 vocab prompts, the W53 na/ka hint, and 'Thais' respelled on 27 English clips. quiz-data/index.json, sentence-hints.json and the unit pages are precached, hence the bump. //   // v723: the W45 gloss-chip audit goes live (QUIZ_REFINEMENT.md W45/W46; CHIP_AUDIT_SYSTEM.md): 2,197 sentences in 111 units re-chipped -- splits, merges, senses, correlatives as two tiles, one-idea frames as one gapped tile, base-form verbs, no articles, house particle forms. Answer keys regenerated and read at answer level (SOLUTION_FINDER.md PHASE 23: split tag questions stay one closer, split time words move as one). Plus W44: 15 published English rewrites (no 'upcountry', Mo Chit, By the way, a really vivid blue, Songkran / Loi Krathong) with new English clips, TE/ET and both stamps, listening decoys and Builder lines re-read. quiz-data/index.json and sentence-hints.json are precached, hence the bump. //   // v722: quiz refinement W41 (QUIZ_REFINEMENT.md W41). Gloss chips that taught a word a sense it does not have (the translation's wording, not the Thai word's: มี 'come in', ใส่ 'has', ใน 'on Google', มา 'for three months') now carry the word's own sense: 167 chips across 63 units, and one merge (วิดีโอคอล + หา -> วิดีโอคอลหา 'video-call', as โทรหา already is). Plus 4 grammar English hints re-picked as spans of their sentence. The unit pages, quiz-data and sentence-hints.json are precached, hence the bump. //   // v721: quiz refinement W40 batch (QUIZ_REFINEMENT.md W40). 30 published English lines corrected after a full read of all 2,457 (owner-approved one by one): meanings (make/have, didn't/wouldn't, watching/playing badminton, 'nicely thought out'/'interesting'), hidden Thai structure (poet-mai 'has just opened', dan, gaw-dai), and spoken brackets/slashes removed. New English clips + TE/ET rebuilt around the unchanged human Thai; listening decoys re-authored for every changed answer; 4 Builder lines, 2 tray tiles, 10 withdrawn Builder answers (the new English names the tile); chip ให้ 'have', ส่วนใหญ่ 'most'. quiz-data/index.json, sentence-hints.json and unit pages are precached, hence the bump. //   // v720: quiz refinement W40 (QUIZ_REFINEMENT.md W40). Two Builder prompts in Places around town 2 put 'new' on the shop where the Thai has poet-mai 'newly opened' on the verb: #1173 (bookshop) reworded, #1177 (bakery) given a prompt, since its English had no word for the poet 'opened' tile. quiz-data/index.json and the unit page are precached, hence the bump. //   // v719: quiz refinement W39 (QUIZ_REFINEMENT.md W39). Vocab Trainer: noun prompts lose a/an/the/a pair of ("knee", not "the knee"), a bracket or the article kept only where the bare word reads wrong; 82 compositional cards removed (ตาแดง 'pink eye', กาแฟร้อน, สวยมาก, สี่คน, ไปวัด...) and their parts carded once each (38 added, 5 moved to a sentence that needs no explainer). quiz-data/index.json and the unit pages are precached, hence the bump. //   // v718: TWO FIXES. (1) THE 'Test page password:' PROMPT (owner's Android app, 'many times', Playlists on the home page and Open player / Enter quiz): playlists.html's test-phase gate, removed at r125, was still being served from thaiear-dl -- a playlist page saved there long ago under its clean /playlists?pl= URL won the navigation fallback whenever the version cache lacked the page (a precache hole), and offline even an exact-URL lookup reached it before the fresh copy. A PRECACHED page's fallbacks now look in CACHE only (precachedNavMatch) and show the offline page rather than a durable copy; topic pages (not precached) keep their thaiear-dl fallback unchanged; the gap-refill retry at the top of the navigation handler is untouched. test_sw_cachefirst 20 (fails on the pre-fix sw.js). (2) THE SCROLL KEEPER moves into topics.js (ThaiEarTopics.scrollKeeper) and is shared by the list pages and playlists.html: a '<- Back to menu' / '<- Back to playlists' tap now lands where you left (one-shot note, 10s), and the playlist list keeps its position on a back-swipe too. topics.js, topics-page.js, playlists.html precached. //   // v711: quiz refinement W30 (QUIZ_REFINEMENT.md W30). Two Builder decoy tiles replaced that could stand in for a real chip against the rewritten prompts (a party for the class reunion; 'when' before reaching the airport). quiz-data/index.json and the unit pages are precached, hence the bump. // v710: quiz refinement W28/W29 (QUIZ_REFINEMENT.md). Builder and Speak Thai literal lines: 64 rewritten that had LOST a cue the published English gives (the learner sees the line as the prompt, so the chip was harder to find than with no line), one redundant line removed, one added (Thai language). Two chip transliterations fixed (yuet, gin khaao). quiz-data/index.json and the unit pages are precached, hence the bump. // v709: quiz refinement W27 (QUIZ_REFINEMENT.md W27). Vocab confusion sweep: prompts that clashed with another word or with their own example sentence reworded (to dial, a free place, a foreign film, right at, an uncle, to put things away, to support, to end it with someone, skill with a first-letter hint); literal notes for hai-yuem, tok-mawn, baw; a pinned example sentence for muu-baan; new VCHIP -- the Vocab card can show a word's standalone gloss on its highlighted chip while the page keeps the sentence-bound one; two corpus chips fixed (move up, Dhamma practice). quiz.js and quiz-data/index.json are precached, hence the bump. // v708: quiz refinement W26 (QUIZ_REFINEMENT.md W26). 24 listening-comprehension decoys replaced that the Thai could not rule out -- they differed from the answer only along axes Thai leaves unmarked (person with a dropped pronoun, tense, number, articles, this/these), so they were a second right answer. Each replacement read against its Thai and its sibling decoys. quiz-data/index.json and the unit pages are precached, hence the bump. // v707: quiz refinement W25 (QUIZ_REFINEMENT.md W25). Vocab glosses standardised: 91 read and changed (event verbs 'to X', nouns singular -- countable 'a X', uncountable 'the X', paired items 'a pair of X'). The fever sentence (Body & health: Symptoms #421): tua-rawn is 'to run a temperature' across chip, vocab, Builder prompt and the published English (new English clip, TE/ET rebuilt around the unchanged Thai, both stamps moved). call-center -> call-centre (British). The Builder reveal now explains na + kha under the model answer. quiz.js, quiz.css, sentence-hints.json and quiz-data/index.json are precached, hence the bump. // v706: quiz refinement W24 (QUIZ_REFINEMENT.md W24; SOLUTION_FINDER.md PHASE 22). A statement closes on falling kha (ka-mai-ek); na or a question lifts it to high kha -- so answers ending a statement on a bare high kha (na dropped or moved) or spelling na + falling kha are refused: 666 removed, all read; key 15,746 -> 15,080. grammar-15: three chips transliterated ka now khaa-high like the other 248. quiz-data/index.json and the unit pages are precached, hence the bump. // v705: quiz refinement W23 (QUIZ_REFINEMENT.md W23; SOLUTION_FINDER.md PHASE 21). Builder key 15,076 -> 15,746 answers, every added one read: the exhaustive remainder of the short-sentence order axis (13,264 candidates read, 251 orders, +616); the order model re-run on the live key (213 outliers read, 4 orders); the drop model's 0.3 tier (40 chips read, pass 15, +37). quiz-data/index.json and the unit pages are precached, hence the bump. // v704: quiz refinement W22 (QUIZ_REFINEMENT.md W22; SOLUTION_FINDER.md PHASE 20). Builder key 13,583 -> 15,076 answers, every added one read: a targeted order sweep over 883 short sentences (object/argument fronted as topic, clause swaps; 5,623 candidates read, 403 orders, +1,095); the drop model's next tier (173 chips read, pass 14, +352); a time word after a question particle where it is natural spoken Thai (wang mai yen-nii; owner ruling, 15 orders, +46). quiz-data/index.json and the unit pages are precached, hence the bump. // v703: quiz refinement W21 (QUIZ_REFINEMENT.md W21; SOLUTION_FINDER.md PHASE 19). Builder key 11,252 -> 13,583 answers, every added one read: 274 single-word omissions on sentences over 7 chips and 230 more found by a new drop-axis model (a moved place phrase keeps its preposition: trap 36, gated); 12 orders from the order model relabelled on the live key and 20 from a 50-sentence blind exhaustive sample. Glosses: jon 'so...that' / 'until' (29). Chips: thii-nai split so yuu-nai / pai-nai are accepted (4). Vocab cards: days & months, colours, kin terms and clock times now live on their category unit. quiz-data/index.json and the unit pages are precached, hence the bump. // v702: ANDROID-ONLY +2px AFTER PAINT, found by device measurement (owner's Pixel 7a, 115% font size; iPhone clean). Android's font-size setting reaches the WebView through Chromium's text autosizer, which enlarges only elements that CONTAIN TEXT -- so an EMPTY reserve sized in em (the cards' listening-time line, 1.3em; the unit-page header row, calc(1.3em + 13px)) resolved against the UNSCALED font, then grew when its text arrived at ~220ms, and the 1fr grid passed +2.3px to every card. A non-breaking space via :empty::before makes each slot text from first paint. No-op at default size (measured: card 156.91 and title 113.3px either way). topics-page.css and player-dyn.css are precached. // v701: the app's premium 'Now playing' bar (nav.js, non-topic pages) wears the premium CARD tint -- var(--card-tint-premium,#F7F4EC) ground, navy ink and eq bars -- instead of the retired gold skin (#FBF5DC / #B29234). Owner 2026-09-25: 'bring it more in line ... with the new tint on the premium topics'. nav.js is precached, hence the bump. //   // v700: quiz refinement, the owner's 2026-09-25 batch (QUIZ_REFINEMENT.md THREAD 9 / W20; SOLUTION_FINDER.md PHASE 18). Builder key +1,796 / -66 answers, every one read: 102 particles/aspect words the drop sweep had vetoed on a word the Builder prompt itself added (at all, keep, together); a closure check (every accepted order in every accepted drop pattern) that found 5 generator bugs; a sentence-final polite particle may also close clause 1 (321 orders); locative thii after yuu; 132 more single-word omissions on short sentences; na 'I wonder' no longer dropped; a time word never fused with a following loei. Listening: didn't/don't decoys over mai-dai removed; grammar units now contrast their sibling structures (32 decoys, 23 tray tiles). Vocab: numerals belong to Time & numbers. quiz-data/index.json is precached, hence the bump. v699: v699: THE UNIT-PAGE HEADER (owner-approved 2026-09-25). The small-caps eyebrow (BEGINNER / STRUCTURE 3 OF 20) is a '<- Back to menu' button, leading to the list actually loaded: the favourites view when the favourites loop is (inFavCircuit, the same test prev/next and the dyn chain use), else /grammar or the unit's band page (topics.js BAND_SLUG, which gen_topics_pages.js now reads too). The favourites heart sits opposite it, 15px in so it centres on the player's right-hand control column. ONE HEART: markup, state, painter, click handler, account load and bfcache restore all live once in topics.js; card and header are two placements of the same button (data-fav-page), and topics-fav.js keeps only the view, the tile count and the circuit (test_favourites.js 9a pins it). The header slot is reserved in render-blocking player-dyn.css at the button's exact height, so the title never moves (measured 113.3px from first paint to filled, topic and grammar, 320-390px, 1x-2x text). Navy on free and premium alike. Playlists: '<- Back to playlists' replaces the eyebrow and the '<- All playlists' link. Card caption: ' . update available' -> ' . update' (owner). // v698: two cosmetic fixes (owner, 2026-09-25). guide.html's mock player now paints its Pauses / Thai speed slider fill from the value (it copied player.js's --te-fill track CSS but never set --te-fill). Topic cards stop changing height as they load: .topic-sent-count is one line (nowrap + ellipsis), so the app's ' · update available' suffix can no longer wrap and grow every card in a 1fr grid (157->176px at 320px); the Favourites view paints its grid at first paint from the favourites mirror when identity.js names the same user the mirror was written for (thaiear_favourites_uid), instead of 'Sign in to keep a list...' until auth.js resolves; Favourites and search cards carry the quiz strip in their markup instead of having it inserted after. PILL_RENDER_MAP.md. // v697: Builder answer key, W19 (SOLUTION_FINDER.md PHASE 17): a shape read of every generated variant not read before -- 736 risky answers in full, 789 samples over 260 shapes. 28 withdrawn, 2 added: a polite particle stacked on a closer (...khaep jing-jing kha kha), na moved past ja/ha (now ...fang yuu na ja), and 4 drops where a nearer subject captures the gap (phom waa + gap = 'I think I...', 'don't move' for 'don't move him'). quiz-data/index.json is precached, hence the bump. v696: player.js r230. A topic Update waits for its verified page save and quiz stamp (capped at 45 s) before it says 'downloaded', the twin of v694's playlist fix. It said 'downloaded' first, so a Buddhism unit needed two presses without leaving the page, and leaving early left the offline copy old (DOWNLOAD_UPDATE_PATH.md fault 13). v695: v695: Builder answer key, W18 (SOLUTION_FINDER.md PHASE 16): all 1,577 answers from the v682 structure sweep read variant by variant; 87 withdrawn, 0 added. The particle-to-end move carried the NOUNS ca 'cost' and kha 'leg' to the end (25); a pronoun before waa 'reckon' was dropped, leaving 'that'/'but' (13); 6 embedded-clause pronouns whose drop hands the clause to the speaker (tham muean phom rai-khaa); 3 verdicts to N. quiz-data/index.json is precached, hence the bump. v694: player.js r229 + dl-core.js. A playlist Update waits for its quiz side-cars to be saved and stamped before it says 'downloaded' (capped at 45 s), and a retry skips side-cars already on disk that match their sig. It announced 'downloaded' first, so the bar flipped straight back to 'update' and a premium playlist on a slow link took several tries (DOWNLOAD_UPDATE_PATH.md fault 12). v693: v693: downloads, faults 10 and 11 (DOWNLOAD_UPDATE_PATH.md). (10) A playlist's Update could never clear its dotted circle: player.js dynPlSaveQuiz read quiz stamps as sig[u] from dynQzSigLoad(), which since r217 returns the WHOLE manifest, so it stamped qz: {} and both the playlist page and the list read that as stale for ever. (11) A topic page's `ver` was stamped from the published map whether or not the page save had succeeded, so the card said current over an old saved copy (Getting to know you). Now `ver` is written only by cachePage() from verified bytes, and both playlist quiz writers go through dl-core.js ThaiEarDL.saveQuizSidecars(), which fetches ?te-save= past the worker, hashes each side-car against its sig, writes only matches, and returns exactly what it saved: the only thing stamped. player.js r228, pl-list.js, dl-core.js. v692: v692: Builder answer key, closeout read (SOLUTION_FINDER.md PHASE 15, QUIZ_REFINEMENT.md W17). Every variant of the statistical drop batches read one by one: 52 answers withdrawn -- a request without noi turned into a permission question (phatlom poet dai mai), duplicate chips made optional twice (duu watch/try, maa come, ja in jon-kwaa-ja), and one maa+laeo pair. quiz-data/index.json is precached, hence the bump. v691: Builder answer key, W15 + W16 (SOLUTION_FINDER.md PHASE 15). A near-duplicate consistency screen: V + maa + a duration + laeo also accepted as V + maa-laeo + the duration (thamngaan maa laeo laai pii), 11 sentences, never with naan; one wrongly accepted order withdrawn (rao phom-waa...). The statistical model now reads a real English parse (AUC 0.87 -> 0.90): 12 more place/time/topic framings. +56 / -2 answers, all read. quiz-data/index.json is precached, hence the bump. v690: Builder answer key, W14 (owner ruling): future ja may be left out where a time word in the SAME clause certainly times the event (phrung-nii pai haa mor-fan kha), never across a clause boundary; 5 sentences, +26 answers, all read; the storm-news sentence kept ja because a fronted phrung-nii would time the news. quiz-data/index.json is precached, hence the bump. v689: Builder answer key, W13 rule hypotheses tested on the corpus (SOLUTION_FINDER.md PHASE 14). nueng + an object classifier inverts (khai loh nueng), never a time unit; a definite object of an instruction fronts as topic (rong-thao thot kon khao baan na khrap), made consistent; a model-ranked screen of rejected candidates found 10 more time/place framings. +119 answers, all read. quiz-data/index.json is precached, hence the bump. v688: Builder answer key, W12 statistical rule mining (SOLUTION_FINDER.md PHASE 13). A model over 48,288 hand-judged candidates (AUC 0.87) pointed the reads: 6 missed orders authored (every night before bed..., every evening after school...); 5 live answers put na straight after a question particle (trap 14) and 15 fronted an object into a dish name (khai thot, kra-thiam jiao..., trap 9) -- all withdrawn and now gated over the whole key; 99 particles and aspect words optional where another tile already carries the English word (maa beside tang-tae 'since', noi beside dai-mai 'could you', duu beside long 'try'), 3 reverted by the two-drop read. Key 9,163 -> 9,490. quiz-data/index.json is precached, hence the bump. v687: Builder answer key, W9 + W10 (SOLUTION_FINDER.md PHASE 11-12). Doubled-meaning pairs: the second carrier of one English word may drop (kam-lang beside yuu, yang 'still' beside gaw/yuu, ...), +61. Sibling search over every rule: 7 time phrases and 6 place phrases that may sit at either clause edge (pi-nii, saao-nii, thaeo-nii mii...); the pronoun subject test now sees 'he'll' and a 'you' after an opener (waa-tae khun...), with 'what about YOU?' and a clause-initial man kept; 33 discourse particles (baang, la, eng, lae, nia, sa, ngai) optional where the prompt renders nothing. 2 more wrong orders withdrawn: a time word fronted out of its own if-clause. Key 8,959 -> 9,163. quiz-data/index.json is precached, hence the bump. v686: Builder answer key, the owner's second rejection-log read (SOLUTION_FINDER.md PHASE 10, QUIZ_REFINEMENT.md W8). C1 loei optional where maak/jang/jon already carries the prompt's really/so (37 sentences); C2 thii-rai optional beside thuk-thii (7); C3 abilitative dai after a quantified complement, khui gan thuk rueang dai (9); C4 emphasis loei on the clause the English intensifies (6). +200 answers, all read, 0 removed. C6: a point-in-time word no longer crosses an if/when opener (it re-timed the condition: 'if it's hot in the morning' vs 'I swim in the morning'), -10 answers in 4 sentences. quiz-data/index.json is precached, hence the bump. v685: the leave-guard audit (LEAVE_GUARDS.md). quiz.js: a quiz arrived at BY URL (every playlist quiz, any ?quiz= link) had no back-swipe guard, so a swipe mid-run silently lost it; a te_run sentinel is now pushed at run start. read.js: the course download gets the back-swipe sentinel, and lesson tests and reading-quiz marks, which had no guard at all, get link-tap + swipe + in-page Exit/Restart/Shuffle/Part confirms. player.js r227. All precached, hence the bump. v684: two Android-app fixes. (1) topics-page.js: the back-navigation scroll restore re-applied itself on `load`, which on a slow link fires seconds late and yanked the learner back after they had started scrolling; any touch/pointer/wheel/key now cancels pending attempts. (2) player.js r226: a back swipe mid-download on a topic page shows the styled leave dialog, not the native beforeunload box. Both files are precached, hence the bump. v683: Builder answer key, time words are clause-intelligent (SOLUTION_FINDER.md PHASE 9, QUIZ_REFINEMENT.md W6). The front/trail move for mobile time words (chao-nii, wan-nii, chuang-nii...) targeted the SENTENCE's start/end, so in two-clause sentences it re-scoped the word onto the other clause: 329 orders in 97 sentences, 100 read. It now moves within its own clause, exits only by fronting over an opening if/when clause, and only a free adverbial moves at all (not after tang-tae/gwaa, inside a relative clause or noun phrase). 332 wrong orders removed, ~70 corrected ones added, 15 reviewed exceptions pinned. Also catches up topic-07 and topic-13b, whose quiz data was a release stale. quiz-data/index.json is precached, hence the bump. v682: v682: Builder answer key, the structure sweep (SOLUTION_FINDER.md PHASE 8). Aspect and particle words (maa, yuu, laeo, gaw, loei, ja, gan, duai, noi, rok, wai, pai, hai) that the English prompt does not render are now optional in the sentences where the Thai stays good without them: 719 hand-read verdicts, 504 optional chips, +1,701 accepted answers across 447 sentences, 0 removed; two-drop combinations read (duration maa + perfect laeo keep one). Plus W2: one grammar Builder decoy re-rolled. quiz-data/index.json is precached, hence the bump. v681: v681: Builder answer key, a class sweep integrated with SOLUTION_FINDER.md (PHASE 7). (1) SOLUTION_FINDER trap 2 found broken live: the particle-to-end move pushed the na of na-kha PAST the kha, so 245 orders in 51 units accepted kha-na / khrap-na endings; a moved na now lands before a closing khrap/kha/kha, and never straight after a question particle (18 more removed). 263 removed, 38 corrected orders added, all read; pinned by test_particle_swap.js. (2) dai 'get to' made optional in 8 sentences whose Builder prompt does not cue it, plus 1 resultative; kin terms used as a subject pronoun optional in 3 (per sentence, never categorial). quiz-data/index.json is precached, hence the bump. v680: v680: two Builder answer-key gaps from the owner's own rejection log (read_quiz_rejections.py, QUIZ_REFINEMENT.md W4): เค้า, the cutesy 'I' between partners, is now a droppable subject pronoun like ผม/ฉัน; and ได้ in มา + ได้ + a duration is optional (คบกันมาสามเดือนแล้ว = คบกันมาได้สามเดือนแล้ว), in Romance & dating 36b #1588 and Introductions #43. topic-02 + topic-36b keys regenerated; quiz-data/index.json is precached, hence the bump. v679: v679: requests carrying ?te-save= are not intercepted (network or failure), and player.js r227 cachePage() uses them: it fetched THROUGH the worker, whose 2 s timeout fallback answered from thaiear-dl itself, so on a slow link it re-saved OLD page bytes under current stamps (owner's phone: 42a, 41b, 13b re-saved old, topic-02 never saved). cachePage now also hashes what it fetched and writes only bytes matching the published pageVer. Guard: test_cachepage_key.js. Plus the passive-marker glosses (dohn/thuuk, QUIZ_REFINEMENT W5), committed underneath this bump by the quiz session; quiz-data/index.json moved. v678: v678: Builder tray decoys, corpus-wide (QUIZ_REFINEMENT.md OPEN WORK W1). 821 placements replaced, every one read before shipping: 654 were the excess of 95 'always wrong' words (a decoy far more often than it is ever a correct tile: ป้าย, ปู่, แค่ไหน...), 167 were shapes the read rejected on sight -- interjections, demonstratives, intensifiers, ก็-fragments, discontinuous chips, a pronoun swap of a tray tile, a gloss already in the prompt -- plus 11 per-sentence synonym vetoes (ปอก beside แกะ, มองหา beside อยากได้, a passive ถูก that rebuilds the same sentence). Rule: quiz_distractor_rules.q2_tile_problem; guards in test_quiz_distractors.py fail on the pre-fix file. quiz-data/index.json moved; precached, hence the bump. v677: quiz.js vocab reveal highlights the word's chip(s) in its example sentence even when the word is no longer ONE chip: the chip that is the word, else the contiguous run that spells it, else the chip containing it. After the chip-split pass an exact-match test left 232 of 3,368 cards highlighting nothing (paet-mong-chao is now paet-mong + chao; the months sit inside duean-X). Guard: test_vocab_highlight.js, which runs the shipped function and fails on the pre-fix quiz.js. Plus 3 topic-01 Builder decoys over the per-unit reuse cap. quiz.js + quiz-data/index.json are precached, hence the bump. v676: six listening decoys from v674 corrected after a clean re-read of the published output: one was a correct translation marked wrong (ran mai = a new shop; a cafe IS a ran), one a near-synonym (laughs it off / lets it go), one echoed a kept decoy, three were implausible. quiz-data/index.json moved; precached, hence the bump. v675: quiz refinement thread 8, the transformed-meaning vocab sweep (VOCAB_SELECTION_RULES.md §5). The redundancy rule cut compounds by how COMMON their parts are and never asked whether the parts ADD UP to the meaning -- khao gan is enter + each other and means get on well. 10 words restored with literal explainers naming their parts (thi ching 'honestly' = at the truth; sak thi 'at last'; suan tua 'personally'...). quiz-data/index.json moved; precached, hence the bump. v674: quiz refinement thread 8, stages 3+4 (QUIZ_REFINEMENT.md). 37 Builder prompts (quiz.enq) rewritten where the full read found bad English: gloss wording lifted in ('confess to him', 'the thoughts you see of yours') or Thai structure mirrored ('If whoever has old clothes'). 122 listening decoys replaced that shifted ONLY an axis the Thai does not mark -- him/her over a bare khao, tense (I love you / I loved you), number, person over a dropped subject -- so both options were right in Thai; now guarded by test_quiz_distractors.py (q1_unmarked_shift). quiz-data/index.json moved with the re-published units; precached, hence the bump. v673: quiz refinement thread 8 (QUIZ_REFINEMENT.md). quiz.js: a Vocab Trainer multi-answer question now forms only when two glosses match IN FULL, so food ("rice dishes") / food ("things to eat") and young (of a man/woman) are separate one-answer questions, each with its own Appears-in sentence (OWNER_TASK_REGISTER H1). quiz-data/index.json moved with the re-published units: 22 corrected corpus glosses (dai/laeo/duai wrong senses, 36b romance chips), 3 new Builder orders (36b #2265, #1597), 66 vocab glosses (38 dam 'to dive' -> 'black', present-tense rule, one's -> your) and 16 restored 22a/22b words. Both files are precached, hence the bump. v672: activate() writes the re-fetch record (thaiear-gaps) BEFORE deleting the old caches, awaited, and the sweep now keeps thaiear-gaps + thaiear-diag. It used to be written after the deletes, fire-and-forget, so a worker torn down in that window kept the rescued OLD bytes with no record to re-fetch them: the owner's Android app ran v671 holding v670's player.js with an empty record until 'Re-download app files'. Plus BREADCRUMBS (SW_ACTIVATE_FIX_PLAN.md §7 Phase 3): install/activate stages, entry counts, timings, rescued paths and each re-fetch's outcome go to thaiear-diag for the owner panel. Counts and paths only. v671: v671: player.js cachePage() saves a topic page under its CLEAN key (pathname, .html dropped) and removes same-path ?query strays only after that save succeeds. It used c.add(location.href), so every re-save from the quiz strip's /topic-NN?quiz=menu landed in a second entry while the clean one an offline card tap is answered from stayed frozen: owner panel measured 21 of 48 downloads holding an OLD page under a CURRENT stamp ("Page update available" offline only; an old two-option vocab question). Playlist pages keep location.href (?pl= identifies them). Guard: test_cachepage_key.js, RED on the pre-fix source. v670: v663: privacy.html -- the reset/clear buttons now DELETE (DATA_PROTECTION.md §4a, open action 6); Last updated 23 Sep. v662: 20 s timeouts + no double-submit on the three delete controls (Progress reset, Read Thai clear, Delete account). v661: progress.html reloads after a fully successful 'Reset progress data', because the in-place repaint kept showing the old quiz scores. v660: auth.js resetQuiz() now erases quiz records through ONE security-definer rpc (erase_quiz_records, erase_progress_records.sql) and treats success as 'rows left === 0'. The per-table DELETEs it replaced were refused by missing grants, so the Progress page's reset never removed a quiz score. read.js's 'Clear my reading progress' likewise now erases read_scores via erase_read_scores() before clearing local (it only ever cleared localStorage). v659: a COMMENT-ONLY correction in pl-list.js, bumped because the file is precached and the rule for that is unconditional. The comment above dlSaveQuizData said a split topic's parts SHARE an audio prefix (topic-13a and 13b 'both ..._LI1'). THEY DO NOT -- MEASURED 2026-09-23: 113 units, 113 DISTINCT prefixes; 13a is BodyHealth_BEG, 13b Health_BEG, 13c Health_LI1. Its CONCLUSION (resolve a unit by sentence number, never by prefix) is still right, for the real reason: a PLAYLIST spans units, so no single prefix can name the unit a given sentence belongs to. ⚠ Corrected in place rather than deleted, because right-for-the-wrong-reason is the most durable kind of wrong -- a later session would re-derive the false premise and might act on it. That premise also underwrote today's prefix-keyed ver/qz baselines, which is why it was measured rather than assumed. v658 is spent.
...
/* ⛔⛔ A PRECACHED PAGE IS ANSWERED FROM THE CURRENT VERSION CACHE, NEVER FROM A DURABLE ONE
   (2026-09-25). Owner's Android app: tapping Playlists on the home page, or Open player / Enter
   quiz on a playlist, "many times" showed a "Test page password:" prompt — the gate playlists.html
   carried in the test phase and lost at r125, months ago.
   How: a navigation to the clean `/playlists?pl=…` that ran past NET_TIMEOUT_MS fell back to
   scopedMatch(), which tried the CLEAN key in EVERY fallback cache — thaiear-dl included, which is
   never version-wiped and where player.js cachePage() saves a playlist page under its own URL —
   BEFORE the `.html` key the fresh precache copy lives under. So an ancient saved playlist page,
   from before the gate was removed, won every slow load, and even an exact-URL offline lookup.
   ✅ For a navigation whose path (or its .html / clean twin) is in PRECACHE, the fallbacks look
   in CACHE ONLY, under the precache key, and NEVER in a durable cache. That copy is the one this
   VERSION shipped; a durable copy of the same page can only be older.
   ⚠ "Never", not "first": the instant path above already reads CACHE, so the ONLY time a precached
   page reaches these fallbacks is when CACHE is MISSING it — a precache hole (SW_ACTIVATE_FIX_PLAN
   §11–13; they happen on real phones). "CACHE first, then durable" would therefore serve the stale
   copy in exactly the case that produced it. With no CACHE copy a slow load waits for the network
   and an offline one gets the offline page — both better than a months-old document.
   Pages NOT precached — the 93 topic pages, whose offline home IS thaiear-dl — never reach this
   and keep their existing lookup exactly.
   Correct for playlists specifically because every playlist is the SAME document: the playlist is
   chosen by the ?pl= query, which the page's own JS reads. */
function precachedNavKey(url) {
  var p = url.pathname;
  if (PRECACHE_PATHS.has(p)) return p;
  var alt = p.slice(-5) === '.html' ? p.slice(0, -5) : p + '.html';
  return PRECACHE_PATHS.has(alt) ? alt : null;
}
function precachedNavMatch(req, url) {
  if (req.mode !== 'navigate') return Promise.resolve(null);
  var key = precachedNavKey(url);
  if (!key) return Promise.resolve(null);
  return caches.keys().then(function (names) {
    if (names.indexOf(CACHE) === -1) return null;      // never CREATE a cache just to look in it
    return caches.open(CACHE).then(function (c) { return c.match(key); });
  }).catch(function () { return null; });
}

function positiveCacheMatch(req, url) {
  if (req.mode === 'navigate' && precachedNavKey(url)) return precachedNavMatch(req, url);   // CACHE or nothing
  return positiveCacheMatchDurable(req, url);
}
function positiveCacheMatchDurable(req, url) {
...
function cacheFallback(req, url) {
  var p0 = url.pathname;
  var isHome = (p0 === '/' || p0 === '/index.html' || p0 === '/index');   // keeps its own two-key lookup below
  if (req.mode === 'navigate' && !isHome && precachedNavKey(url)) {
    return precachedNavMatch(req, url).then(function (pre) {
      return pre ? cleanRedirect(pre) : offlinePage();   // the shipped copy or the offline page — never a durable one
    });
  }
  return cacheFallbackDurable(req, url);
}
function cacheFallbackDurable(req, url) {
...
self.addEventListener('fetch', function (e) {
  ...
  /* ⛔⛔ v679 — A DOWNLOAD SAVE IS NEVER ANSWERED FROM A CACHE. player.js cachePage() fetches
     `?te-save=` to persist a topic page offline. Through the normal path, a response slower than
     NET_TIMEOUT_MS fell back to positiveCacheMatch() — ignoreSearch, thaiear-dl included — so the
     "save" copied an OLD saved page back into itself (owner's Android app, 2026-09-24: three units
     re-saved with their old ?quiz=menu bytes under current stamps). Not intercepting at all means
     network or failure, and cachePage writes nothing on failure. Also keeps the busted URL out of
     the version cache. */
  if (url.searchParams.has('te-save')) return;
