/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v595';   // v595: the head-start control is shown on the TRAY's chip count, not the corpus one. quiz.js is precached and cache-first, so this bump is what delivers it; v594 is spent. ⚠ v594 wired the Builder's tray to topic.quiz.bchips but left the count that DECIDES whether the head-start control appears reading chipsOf(). They can disagree: a unit whose largest sentence is 6 corpus chips, one of which splits into 7 tiles, hides the control (6 > 6 fails) while qBuild applies the cap against canon = 7 and sets lockN = 1 — a head start firing on a setting the learner was never shown and could not have chosen. That is the exact inversion of the rule the control exists for. ⚠ INERT until a unit both carries bchips and sits on the boundary, and neither pilot carries bchips — bumped anyway because a precached file that changes without a bump is served stale INDEFINITELY, so the fix would still be missing from those clients on the day it matters. Found by a parallel session reviewing v594.
