/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v558';   // v558: about.html sheds the two paragraphs that were doing guide.html's job. It had drifted into a second, shorter guide: it enumerated the dyn player's controls (pause length, repeats, English on/off, Thai speed) -- which guide.html demonstrates with a LIVE working player -- and it defined shadowing and active recall with double glosses, under a heading guide.html already owns, then said "(see Guide)" twice in the same page. Now About states the principle and hands over: the player paragraph keeps the App link and points at Guide with a reason ("a working player you can try"), and the listening-modes paragraph keeps both TERMS once, states that Thai-first builds understanding while English-first builds speaking, and defers the how-to. 386 -> 349 visible words; two paragraphs changed and nothing else -- the opening, the positioning paragraph, the three research bullets, the implementation paragraph, contact and the Krashen/Paivio/Roediger citations are untouched. "Active recall" deliberately survives in the research bullet: About states the PRINCIPLE with evidence, Guide teaches the TECHNIQUE. Measured first -- /about and /guide have 15 and 12 impressions and ZERO clicks in 90 days, so merging the two pages was considered and rejected on what they do for a reader, not on search. about.html is PRECACHED, hence the bump. // v557: THE THAI SPEED STOPS MOVE 100/90/80 -> 100/92/84. Owner, 2026-09-19: at 80% a long sentence dragged, and "I'd rather sentences can't get as slow as people might like rather than have them sound poor quality (given they were expensive from a professional studio)" -- the stretch is a synthesis artefact laid over a paid studio recording, so the floor is raised rather than the artefact tolerated. The slider shows no number, so no copy changes anywhere. THE PART THAT IS NOT JUST NUMBERS: a preference already saved as 0.9 or 0.8 is still a finite in-range value, so nothing rejects it -- but it is no longer in SP_STEPS, so indexOf gives -1, the slider falls back to index 0 and reads "Normal" while the session keeps building at the old rate. dynSnapSp() now moves any stored value onto the nearest current stop (0.9 -> 0.92, 0.8 -> 0.84), so whoever chose the slowest keeps getting the slowest, and it is applied at all three read sites. SP_STEPS was three separate literals and is now one module-scope constant. The session key already carries |s<rate>, so a session built at the old speed self-invalidates and rebuilds; anyone who downloaded a unit at 90/80% rebuilds it once. Accepted range stays 0.8..1 so a record from an older build is still read as a real choice. player.js is precached, hence the bump. player.js r220, test_dyn_speed.js 52.
