/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v649';   // v649: ONE NEW ACCEPTED WORD ORDER ON topic-14a #510, shipped as a LIVE TEST of the update path the last six releases built. วันนี้ดีใจมากเลยค่ะ | สอบผ่านแล้ว ("I'm so happy today - I passed the exam") already accepted วันนี้ at the SENTENCE end; it now also accepts it at the end of CLAUSE 1, before the particle: ดีใจ · มากเลย · วันนี้ · ค่ะ · สอบ · ผ่าน · แล้ว. From the fronted-time-adverbial class in QUIZ_REFINEMENT.md thread 2; the politeness clamp is what puts วันนี้ BEFORE ค่ะ rather than after it. 6 accepted orders became 9, not 7: an authored order is re-entered through acceptedOrders() as a pseudo-chip list so it picks up every axis by construction, which is where its no-ค่ะ twin and its ค่ะ-to-end variant come from. ⚠⚠ THE CHAIN IS FOUR STEPS AND I GOT IT WRONG TWICE: quiz-orders-all.json -> export_quiz_orders.py -> gen_quiz_orders.js -> gen_quiz_data.js. Running gen_quiz_data alone does NOTHING to the key (it PRESERVES orders via keepOrders and never derives them), and reporting that as "rejected by a derivation gate" was a wrong conclusion, not a finding. ✅ BOTH STAMPS MOVED, which is the point of shipping it: the published per-unit sig fhrw20p7pi -> 3em1f3tpmp (what the CARD compares) and the page's contentHash 19w9k60zgnk -> 28wbrv93ihs (what the PAGE compares). A downloaded topic-14a, and any playlist holding its sentences, must now offer an update on every surface with no user action -- that is the whole thing being measured. quiz-data/index.json is precached, hence the bump. v648 is spent.
