/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v641';   // v641: A RE-PUBLISHED QUIZ CAN NOW REACH A DOWNLOAD -- and a downloaded topic could not see one AT ALL. Owner, 2026-09-22: "for people who have downloaded topic pages, they won't be able to see the quiz - they will see just the old topic page". THREE separate gaps, each invisible: (1) contentHash() hashed num+thai+english only, so a page that gained an entire quiz arm hashed IDENTICALLY to the page before it; it now folds in the quiz block, which also means a Solution-Finder acceptance or a C3 prompt rewrite MOVES it -- orders and enq live there and nothing else would ever have noticed them. (2) the baseline was never recorded on the path that matters: markDownloaded() stamps `ver` and only the CLASSIC download calls it, so EVERY dyn download -- which is every download on the site -- landed with ver:'' and refreshOfflineState()'s text check was not merely wrong but UNREACHABLE. (3) the PLAYER's own playlist download never saved the quiz side-cars at all (pl-list.js's batch download has since v600), so a playlist downloaded from the ?pl= page had every clip and no questions the moment it went offline. A playlist is the case that cannot self-heal: pl-quiz.js's loadUnit() reads the durable thaiear-dl copy FIRST and UNCONDITIONALLY, which is right and means nothing in the product could ever have replaced it -- so both the row and the player now compare per-unit stamps from quiz-data/index.json against a per-playlist baseline, mirroring r137's audio design rather than inventing a second mechanism. ONE DELIBERATE ASYMMETRY: an ABSENT quiz baseline reads as stale (r137 treats an absent AUDIO one as fine). Those are exactly the downloads the owner asked about, it is one prompt, and it is self-clearing. ALSO, owner 2026-09-23: "page update should supersede the audio update message ... but please dont let there be a render race / ugly button flash". Ranking the two messages could not have settled it -- the audio check is async and painted bar.innerHTML DIRECTLY, so whoever painted second won whatever the ranking said. Neither check paints now: each raises a flag and one painter renders the winner, so arrival order stops mattering, and an identical render is skipped outright so a late signal touches no DOM. ADDING THE QUIZ TO THE HASH RE-FLAGS EVERY EXISTING TOPIC DOWNLOAD ONCE -- correct rather than a side effect, since every one of them genuinely is missing the quizzes. player.js, pl-list.js, topics-page.js and quiz-data/index.json are all precached and served cache-first, so without this bump no returning device gets any of it. v640 is spent.
