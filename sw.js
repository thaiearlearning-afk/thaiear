/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v651';   // v651: THE C3 PASS LANDS -- 92 of 113 units get rewritten quiz PROMPTS. The C3 queue is CLOSED at 0 candidates: 185 verdicts recorded this pass, ~130 rewrites and ~55 declines, on top of the 450 overrides that were already live. ⚠ ONLY THE QUIZ PROMPT CHANGES -- the published translation under each sentence is untouched, which is the whole point of the bchips/enq split (QUIZ_REFINEMENT.md rule 2): the corpus is approved content, the prompt is a Builder input, and they are allowed to differ. WHAT A REWRITE FIXES: a chip the learner had NO LEXICAL ROUTE TO -- ขอ='may I have' dropped for a bare 'please', ให้ได้='make sure to' flattened to 'absolutely', กว่าจะ='it takes a lot to' to 'pretty hard', ตุบๆ (ONOMATOPOEIA) to 'beating', and two chips compressed into one English word (ริม+ถนน into 'street', ดู+จบ into 'binged'). One was a plain spelling mismatch: the chip is glossed 'Mo Chit' and the prompt wrote 'Mochit'. ⛔ WHAT WAS DECLINED, and the distinction matters: a Thai PARTICLE can never be cued by any natural English sentence -- directional ไป/มา, aspectual แล้ว, correlative ก็, comparative กว่า, classifiers คู่/มัด/ชิ้น, question เหรอ/ไหม, emphatic เนี่ย -- and a rewrite that tries teaches the gloss's literal AID as if it were the meaning. A synonym decline says a route exists; a particle decline says none can. ⭐ THREE DETECTOR BUGS were found by READING candidates, not by reviewing code, and each looked exactly like a rule being merely conservative: a trailing possessive 's was never stripped; the corpus uses CURLY apostrophes so "didn’t" tokenised as 'didn'+'t' and skipped the contraction list; and the inflection test was gated on len>3, so every THREE-letter gloss (ใช้='use') could never suppress its own inflection. ✅ VERIFIED AFTER REGEN: the order key still matches the chips, and test_quiz_key_wiring asserts 7,225 accepted orders and 96,942 that must be rejected across all 113 units. ⚠⚠ THIS FLAGS EVERY DOWNLOADED TOPIC ONCE, on the card and on the page, because contentHash and 92 per-unit sigs all move -- that is the v641-v649 update path doing exactly what it was built for, proven on the owner's device at v649, and it is why this was held until he asked for it. quiz-data/index.json is precached. v650 is a peer's and is spent.
