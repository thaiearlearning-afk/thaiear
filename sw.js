/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v654';   // v654: READ THAI RESULTS ARE ON THE ACCOUNT, and the legal text catches up with the quizzes. (1) Owner: "i didnt know someones read thai scores arent saved". They were localStorage only. read_scores + apply_read_scores() (tooling 84f51d7, applied 2026-09-23) now hold them; read.js queues a DELTA on every recorded result and pulls the account copy on thaiear:auth. ⚠ FLUSH THEN PULL, in that order: a pull that overwrote local while a delta was still queued would discard the attempt, so ThaiEarReadStore.pull() drains the outbox first and only then treats the server totals as authoritative. ⚠ An ABSENT server row leaves the local one alone -- that is what stops the first sign-in wiping a device's own history. ⚠ A one-time per-device migration queues pre-existing local totals, flag-guarded because these are COUNTERS and queueing twice would double the attempt count. (2) ⛔ NO PER-ATTEMPT DATES ARE SYNCED. read.js writes lastCorrect/lastTotal/lastDate and NEVER READS THEM BACK; carrying them up would mean holding a dated record of when each of ~24 tests was last taken, for a feature that does not exist. PLAYS_PRIVACY_BRIEF.md §7 makes the same call for listening. privacy.html now says so in as many words. (3) The readsc outbox op DROPS ITSELF when apply_read_scores is absent (shipped in the peer's v653). Retrying would jam a queue shared with quiz scores, prefs, exclusions, item stats and Builder rejections -- the v617 shape. ⛔ "Apply the migration first" was the wrong fix: it works once and leaves a permanent ordering dependency for the next client-ahead-of-migration. Only a MISSING FUNCTION is swallowed; RLS, network and bad-row errors still retry. (4) terms.html: "two parts" -> THREE (Grammar by Ear has been public since 2026-08-29 and was never listed); quizzes added to what a free account unlocks; §3 now says Premium unlocks the audio AND QUIZZES, because a premium unit's quizzes are premium too and the old wording could be read as quizzes being free everywhere; §9 rewritten -- quiz scores and Read Thai results are both account-stored and both sync after being offline. (5) progress.html gains the meta description it never had. playlists.html and account.html still have none -- pre-existing, logged, not fixed here. ⚠ A DEAD REPAINT WAS REMOVED BEFORE IT SHIPPED: the pull first called render() behind a `typeof render === 'function'` guard, and both of read.js's render functions are INNER, so it could never fire. There is deliberately no forced repaint now -- boot() would rebuild the view and throw away a test in progress. read.js, auth.js, privacy.html, terms.html and progress.html are precached and cache-first. v653 is a peer's and is spent.
