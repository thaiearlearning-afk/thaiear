/* ============================================================
   sw.js â€” ThaiEar service worker (offline app shell + pages).
   ------------------------------------------------------------
   ...
   content â€” the tandem-update model is preserved â€” and the cache is only
   ...
   ============================================================ */
const VERSION = 'v33';   // v33: dyn round 6 - plsel topic-selection redesign, factory control parity, iOS fixes (2026-07-27)
...
function offlinePage() {
  var html = '<!doctype html><html lang="en"><head><meta charset="utf-8">' +
    '<meta name="viewport" content="width=device-width,initial-scale=1"><title>Offline Â· ThaiEar</title><style>' +
    ...
    '</style></head><body><div class="b"><h1>Youâ€™re offline</h1>' +
    '<p>This page isnâ€™t available without a connection.</p>' +
...
// Positive cache lookup (returns a Response or null â€” NEVER the offline page), used by the timeout
...
function cacheFallback(req, url) {
  return caches.match(req).then(function (hit) {
    ...
    if (req.mode === 'navigate') {
      ...
      // Match by PATHNAME, ignoring any query string (?next=, ?feature=1, ?sub=successâ€¦), then the
...
const PRECACHE = [
  '/', '/index.html',
  // Navigable non-topic "shell" pages â€” precached so they open offline (each renders its own
  ...
  // shared assets. Audio is NOT precached (208 clips on the CDN â€” cached on play).
...
self.addEventListener('activate', function (e) {
  e.waitUntil(
    caches.keys()
      ...
      // downloaded-AUDIO cache ('thaiear-audio-dl', the web/PWA offline-audio store) â€” neither
...
self.addEventListener('fetch', function (e) {
  ...
  // (Fonts used to be Google-hosted and cached here; they're self-hosted now â€” handled same-origin.)
  ...
  // rejects â€” which made cached pages slow to appear. So if the network hasn't answered in time AND
