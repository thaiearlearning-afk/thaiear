/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v593';   // v593: a REPLAY inside a quiz now counts as a second listen. quiz.js is precached and cache-first, so this bump delivers it; v592 is spent. Owner, 2026-09-21: "i played the thais like to snack all day sentence about [several] times on the vocab trainer. so that should be seen as multiple plays, not just one." It counted ONCE. ⚠⚠ CAUSE: toggleSentence() resumes the ALREADY-LOADED element and therefore reaches neither playSentence() (a fresh clip) nor stopAudio() (teardown) — the only two places auCredited was reset — so every replay after the first was silently uncounted. Inconsistent with the dyn player, where one trip through a card at Thai-repeats 4 is ONE pass and FOUR listens: same repetition, different accounting. ✅ The `ended` rewind branch now re-arms the credit. ⛔ THAT BRANCH ONLY, and the distinction is the point (owner agreed): replaying a clip that FINISHED is a genuine second listen; pausing mid-clip and resuming is ONE listen interrupted, and crediting it would let anyone inflate the one number on this site that is honestly measured just by tapping pause/play. ⚠ Found by the owner's offline test session, which also closed §11A items 6 and 7: quiz plays credit and sync (+16 reps/+16 passes/+56.9s from an aeroplane-mode run), and all four quizzes play offline on v592.
