/* ============================================================
   ...
   ============================================================ */
const VERSION = 'v656';   // v656: THE CARD CAN SEE THE PAGE NOW -- a per-unit PAGE HASH is PUBLISHED beside the quiz stamp, which closes the FOURTH card/page divergence and the one stale_matrix.js could not catch. Owner, 2026-09-23, on topic-04b: card said downloaded, page said update available. Both were right about the only field each could see -- qz matched published, ver did not match the live page. ⭐⭐ THE CAUSE IS A WRITE-SIDE SKEW, NOT A READ-SIDE ONE, AND THAT IS WHY MY TABLE MISSED IT: stale_matrix.js asserts the two READERS agree given the same fields, while download finalize was filling those fields from DIFFERENT EPOCHS -- `ver` by hashing the page IN MEMORY (which can be a stale cached copy) and `qz` by FETCHING the published sig (always current). Testing the readers moved the divergence to the writer rather than removing it. ✅ gen_quiz_data.js now publishes `ver` per unit in quiz-data/index.json and topics-page.js compares against it exactly as it does the quiz sig, so both surfaces answer from PUBLISHED data. This also retires the asymmetry v645 documented as acceptable under-reporting -- a text-only edit moving contentHash but not the side-car was invisible to the card, and 'accepted' stopped being the right word once it produced a visible contradiction. ⛔⛔ THE HASH CONSTRUCTION IS LOAD-BEARING AND MUST MATCH player.js BYTE FOR BYTE: num/thai/english joined by U+0001, sentences by U+0002, then U+0003 and the quiz block as JSON (player.js hashes cfg.sentences UNMODIFIED, which is why reading the page's own data block reproduces it). If those ever drift, every downloaded device flags an update that re-downloading CANNOT clear, for ever -- so test_topics_page now recomputes it from each page's own data block and asserts equality on ALL 113 units. ✅ Proven red before green: the new stale_matrix row fails against the pre-fix topics-page.js. topics-page.js and quiz-data/index.json are precached. v655 is spent.
